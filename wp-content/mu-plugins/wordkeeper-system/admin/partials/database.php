...
<div class="wrap-page-admin-panel">
		...
		<ul class="tabs">
			<?php foreach ($tabs as $tab_slug => $tab): ?>
				...
				<li class="item<?php echo esc_attr($active_class); ?>">
					<a href="<?php echo esc_attr($site_path); ?>wp-admin/<?php echo ($network) ? 'network/' : ''; ?>admin.php?page=wordkeeper-system-database&tab=<?php echo esc_attr($tab_slug); ?>"><?php echo esc_html($tab['title']); ?></a>
				</li>
				<?php endif; ?>
			<?php endforeach; ?>
		</ul>
	...
	<div class="databases">
		<?php if($current_tab): ?>
			<?php if(file_exists(dirname(__FILE__) . '/tabs/database/' . $current_tab . '.php')): ?>
				<?php require 'tabs/database/' . $current_tab . '.php'; ?>
			<?php endif; ?>
		<?php endif; ?>
	</div>
</div>
