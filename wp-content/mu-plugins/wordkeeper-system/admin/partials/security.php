...
<div class="wrap-page-admin-panel">
		...
		<ul class="tabs">
			<?php foreach($tabs as $tab_slug => $tab): ?>
				...
				<li class="item<?php echo esc_attr($active_class); ?>">
					<a href="<?php echo esc_attr($site_path); ?>wp-admin/<?php echo ($network) ? 'network/' : ''; ?>admin.php?page=wordkeeper-system-security&tab=<?php echo esc_attr($tab_slug); ?>"><?php echo esc_html($tab['title']); ?></a>
				</li>
				<?php endif; ?>
			<?php endforeach; ?>
		</ul>
	...
	<?php if($current_tab): ?>
		<div class="holder-postbox-widget">
			<?php if(file_exists(dirname(__FILE__) . '/tabs/security/' . $current_tab . '.php')): ?>
				<?php require 'tabs/security/' . $current_tab . '.php'; ?>
			<?php endif; ?>
		</div>
	<?php endif; ?>
</div>
