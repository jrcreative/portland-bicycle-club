...
class RML_View {
    ...
    public function optionsHTML($selected = -1, $tree = null, $slashed = "", $spaces = "--", $useAll = true, $disabled = null) {
        ...
        if ($tree == null) {
            ...
            $return .= '<option value="-1" ' . $this->optionsSelected($selected, "-1") . '
                            ...
                            data-id="-1">' . __('Unorganized pictures', RML_TD) . '</option>';
        }
    ...
    public function treeHTML($selected = -1, $tree = null, $list = "") {
        ...
        if ($tree == null) {
            $tree = $this->structure->getTree();
            /* @deprecated
            $return .= $this->createNode(-1, RML_TYPE_ROOT, "/", "/ " . __('Unorganized', RML_TD), $this->structure->getCntRoot(),
                                $selected, array(), $list);
            */
        }
        ...
        if(!is_null($tree) && count($tree) > 0) {
            foreach($tree as $parent) {
                ...
                $return .= $this->createNode($parent->id, $parent->type, "/" . $parent->absolutePath(), $parent->name, $parent->getCnt(),
                                $selected, array(), $list);
                ...
                if (isset($parent->children) &&
                    ...
                    $return .= $this->treeHTML($selected, $parent->children, $list);
                }else{
                    $return .= '<ul></ul>';
                }
     ...
     */
    public function createNode($fid, $type, $slug, $name, $cnt, $currentFid, $classes = array(), $list = "") {
        // Get href
        ...
        /* @deprecated
        return '
        <a href="' . $href . '" class="' . $classes . '" ' . $slug . ' data-type="' . $type . '" data-id="' . $fid . '">
            ' . $icon . '
            <div class="rml-name">' . $name . '</div>
            ' . apply_filters('RML/Folder/TreeNode/Content', "", func_get_args()) . '
            <span class="rml-cnt rml-cnt-' .  $cnt . '">' . $cnt . '</span>
        </a>
        ';
        */
        return '
        <a href="' . $href . '" class="' . $classes . '" ' . $slug . ' data-aio-type="' . $type . '" data-aio-id="' . $fid . '">
            ' . $icon . '
            <div class="aio-node-name">' . $name . '</div>
    ...
    public function treeHref($id, $type, $list = "") {
        $query = $_GET;
        $query['rml_folder'] = $type === RML_TYPE_ALL ? "" : $id;
        ...
        if(!is_null($tree) && count($tree) > 0) {
            foreach($tree as $parent) {
                ...
                if (isset($parent->children) &&
                    ...
                    $append = $this->namesSlugArrayRec($parent->children, $spaces . "--");
