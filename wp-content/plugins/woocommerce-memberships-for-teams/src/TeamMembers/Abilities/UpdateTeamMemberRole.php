...
/**
 * Ability: Update Team Member Role.
 *
 * Changes a team member's role.
 *
 * @since 1.8.0
 */
class UpdateTeamMemberRole implements MakesAbilityContract
{
	const NAME = 'woocommerce-memberships-for-teams/team-members-update-role';
	...
	public function makeAbility() : Ability
	{
		return new Ability(
			static::NAME,
			__('Update Team Member Role', 'woocommerce-memberships-for-teams'),
			__('Changes a team member\'s role.', 'woocommerce-memberships-for-teams'),
			...
			function (array $params) {
				...
				try {
					$member->set_role($params['role']);
				} catch (Framework\SV_WC_Plugin_Exception $e) {
					return new WP_Error('update_role_failed', $e->getMessage(), ['status' => 422]);
				}
			...
			[
				...
				'properties' => [
					...
					'user_id' => [
						'type'        => 'integer',
						'required'    => true,
						'description' => __('The user ID of the team member.', 'woocommerce-memberships-for-teams'),
						'minimum'     => 1,
					],
					'role' => [
						'type'        => 'string',
						'required'    => true,
						'description' => __('The new role for the team member (e.g. "member", "manager").', 'woocommerce-memberships-for-teams'),
					],
				],
			],
			...
			true,
			new RestConfig('/teams/(?P<team_id>\d+)/members/(?P<user_id>\d+)/role')
		);
	}
}
