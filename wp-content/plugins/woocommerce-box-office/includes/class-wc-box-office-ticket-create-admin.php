...
class WC_Box_Office_Ticket_Create_Admin {
	...
	public function __construct() {
		$this->_fields = array(
			...
			'create_order_method' => array(
				...
				'error_message' => esc_html__( 'Please choose create order method', 'woocommerce-box-office' ),
			),
			'ticket_order_id' => array(
				'sanitize'      => 'absint',
				'required'      => array( $this, '_maybe_require_order_id' ),
				'validate'      => array( $this, '_validate_existing_order' ),
				'error_message' => esc_html__( 'Invalid order ID provided', 'woocommerce-box-office' ),
			),
			'quantity' => array(
				'sanitize'      => 'absint',
				'required'      => true,
				'validate'      => array( $this, '_validate_not_zero' ),
				'error_message' => esc_html__( 'Ticket quantity must be greater than zero', 'woocommerce-box-office' ),
			),
			'product_id' => array(
				'sanitize'      => 'absint',
				'required'      => true,
				'validate'      => array( $this, '_validate_product' ),
				'error_message' => esc_html__( 'Please choose ticket-enabled product', 'woocommerce-box-office' ),
			),
			'ticket_fields' => array(
	...
	private function _process_posted_data( $posted_data, $step ) {
		if ( ! empty( $posted_data ) && ! check_admin_referer( 'create_event_ticket' ) ) {
			throw new Exception( esc_html__( 'Error - please try again', 'woocommerce-box-office' ) );
		}
	...
	private function _maybe_create_order_item() {
		...
		if ( $order_id ) {
			...
			if ( ! $item_id ) {
				throw new Exception( esc_html__( 'Error: could not create order item', 'woocommerce-box-office' ) );
			}
	...
	private function _is_field_required( $field, $field_prop, $data ) {
		...
		if ( $is_required && ! isset( $data[ $field ] ) ) {
			throw new Exception( esc_html__( 'Missing value for field %s', 'woocommerce-box-office' ) );
		}
	...
	private function _validate_field( $field, $field_prop, $data ) {
		...
		if ( $validator && ! call_user_func_array( $validator, array( $this->_clean_data[ $field ], $data ) ) ) {
			$message = ! empty( $field_prop['error_message'] ) ? $field_prop['error_message'] : sprintf( esc_html__( 'Something wrong with field %s.', 'woocommerce-box-office' ), esc_html( $field ) );
	...
	private function _validate_product( $value, $data ) {
		...
		if ( 'no_order' !== $data['create_order_method'] ) {
			if ( ! $product_check->is_purchasable() ) {
				throw new Exception( esc_html__( 'The product cannot be purchased.', 'woocommerce-box-office' ) );
			}

			if ( $product_check->is_sold_individually() && $qty > 1 ) {
				throw new Exception( esc_html__( 'Product is set to sold individually. You can only add one ticket.', 'woocommerce-box-office' ) );
			}

			if ( ! $product_check->is_in_stock() ) {
				throw new Exception( sprintf( esc_html__( 'You cannot create ticket for &quot;%s&quot; because the product is out of stock.', 'woocommerce-box-office' ), esc_html( $product_check->get_title() ) ) );
			}

			if ( ! $product_check->has_enough_stock( $qty ) ) {
				throw new Exception( sprintf( esc_html__( 'You cannot create that amount of ticket(s) because there is not enough stock (%s remaining).', 'woocommerce-box-office' ), esc_html( $product_check->get_stock_quantity() ) ) );
			}
		}
	...
	private function _get_variation_id( $product, $data ) {
		...
		if ( ! $variation_id ) {
			throw new Exception( esc_html__( 'Please select the product options.', 'woocommerce-box-office' ) );
		}
		...
		foreach ( $attributes as $name => $values ) {
			...
			if ( empty( $data[ $key ] ) ) {
				throw new Exception( sprintf( esc_html__( 'Please select option for %s.', 'woocommerce-box-office' ), esc_html( $name ) ) );
			}

			$value = wc_clean( $data[ $key ] );
			if ( ! in_array( $value, $values ) ) {
				throw new Exception( sprintf( esc_html__( 'Invalid value for option %s.', 'woocommerce-box-office' ), esc_html( $name ) ) );
			}
	...
	private function _validate_ticket_fields( $value, $data ) {
		...
		if ( $this->_clean_data['quantity'] !== sizeof( $data['ticket_fields'] ) ) {
			throw new Exception( esc_html__( 'Number of ticket fields does not match with provided ticket quantity.', 'woocommerce-box-office' ) );
		}
