...
class WC_Box_Office_Report {
	...
	public function reports_tab( $reports ) {
		if ( ! WC_Box_Office::display_reports() ) {
			// If COT is enabled but not synced, we don't want to show the reports tab.
			return $reports;
		}

		if ( is_admin() ) {
			$reports['tickets'] = array(
				'title'   => esc_html__( 'Tickets', 'woocommerce-box-office' ),
				'reports' => array(
					'sales_by_product' => array(
						'title'       => esc_html__( 'Sales by Product', 'woocommerce-box-office' ),
						'description' => '',
						'hide_title'  => true,
						'callback'    => array( $this, 'sales_by_product' ),
					),
				),
			);
		}
	...
	public function sales_by_product() {
		...
		foreach ( wc_box_office_get_all_ticket_products() as $product ) {
			...
			if ( $product->is_in_stock() ) {
				$stock = '<mark class="instock">' . esc_html__( 'In stock', 'woocommerce-box-office' ) . '</mark>';
			} else {
				$stock = '<mark class="outofstock">' . esc_html__( 'Out of stock', 'woocommerce-box-office' ) . '</mark>';
			}
