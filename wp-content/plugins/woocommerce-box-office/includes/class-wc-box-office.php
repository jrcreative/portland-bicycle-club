...
class WC_Box_Office {
	 ...
	 * @var bool
	 */
	protected $display_reports = true;
	...
	private function __construct( $file, $version ) {
		define( 'WOOCOMMERCE_BOX_OFFICE_ABSPATH', trailingslashit( plugin_dir_path( $file ) ) );
	...
	public function __get( $name = '' ) {
		if ( 'display_reports' !== $name ) {
			return null;
		}

		if ( WP_DEBUG ) {
			$message = sprintf(
				/* translators: %1$s: Property name, %2$s: Version number, %3$s: Method name */
				__( 'The %1$s property is deprecated since version %2$s, call the method %3$s instead.', 'woocommerce-box-office' ),
				'<code>WC_Box_Office::$display_reports</code>',
				'1.3.5',
				'<code>WC_Box_Office::display_reports()</code>'
			);
	...
	/**
	 * Check if reports should be displayed.
	 *
	 * @since 1.3.5
	 *
	 * @return bool True if reports should be displayed, false otherwise.
	 */
	...
	public static function display_hpos_incompatibility_notice() {
		...
		if ( current_user_can( 'activate_plugins' ) && ! self::display_reports() ) {
