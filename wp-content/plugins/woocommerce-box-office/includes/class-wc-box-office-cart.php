...
class WC_Box_Office_Cart {
	...
	public function change_add_to_cart_text_single( $text, $product ) {
		if ( wc_box_office_is_product_ticket( $product ) ) {
			$text = apply_filters( 'woocommerce_box_office_add_to_cart_text_single', esc_html__( 'Buy Ticket Now', 'woocommerce-box-office' ) ); // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingHookComment
		}
		return $text;
	}
	...
	public function filter_cart_message( $message, $product_id ) {
		...
		$added_text = sprintf(
			_n( '%s"%s" has been added to your cart.', '%s"%s" have been added to your cart.', $qty, 'woocommerce-box-office' ), // phpcs:ignore
			$qty > 1 ? $qty . '&times; ' : '',
