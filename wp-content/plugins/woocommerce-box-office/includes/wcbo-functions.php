 ...
 * @param int|string|null $amount  Number of tickets to fetch. Defaults to value of `posts_per_page` option.
 * @param int             $page    Page to fetch.
 *
 * @return WP_Query Query object with tickets.
 */
function wc_box_office_query_tickets_by_user( $user_id = 0, $amount = null, $page = 1 ) {
	if ( ! $user_id ) {
	...
	if ( null === $amount ) {
		$amount = get_option( 'posts_per_page', 10 );
	} elseif ( 'all' === $amount ) {
		$amount = -1;
	}
	...
	/**
	 * Filter the query arguments for tickets by user.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	$args = apply_filters(
		$plugin_token . '_user_tickets_query',
		array(
			// Filters suppressed for back-compat with wc_box_office_get_tickets_by_user() which previously used get_posts().
			'suppress_filters' => true,
			'post_type'        => 'event_ticket',
			'post_status'      => 'publish',
			'posts_per_page'   => $amount,
			'paged'            => $page,
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- required as user ID is not necessarily the post author.
			'meta_query'       => array(
				array(
					'key'   => '_user',
					'value' => $user_id,
 ...
 * @param  int|string $amount  Number of tickets to fetch. Defaults to 'all'.
 * @param  integer    $page    Page to fetch.
 *
 * @return WP_Post[] Array of ticket post objects.
 */
function wc_box_office_get_tickets_by_user( $user_id = 0, $amount = 'all', $page = 1 ) {
	$query = wc_box_office_query_tickets_by_user( $user_id, $amount, $page );
...
function wc_box_office_get_parsed_ticket_content( $ticket_id, $content ) {
	...
	// Parse post vars: '{post_title}', '{post_content}', etc. The post object
	...
	if ( $product_id ) {
		...
		$post_vars = array(
			...
			'{product_price}' => wc_price( $ticket_product->get_price() ),
			'{product_sku}'   => $ticket_product->get_sku(),
		);
...
function wc_box_office_ticket_field_types() {
	...
	$types = array(
		'first_name' => esc_html__( 'First name', 'woocommerce-box-office' ),
		'last_name'  => esc_html__( 'Last name', 'woocommerce-box-office' ),
		'email'      => esc_html__( 'Email address', 'woocommerce-box-office' ),
		'url'        => esc_html__( 'URL', 'woocommerce-box-office' ),
		'twitter'    => esc_html__( 'Twitter', 'woocommerce-box-office' ),
		'text'       => esc_html__( 'Text', 'woocommerce-box-office' ),
		'select'     => esc_html__( 'Drop down menu', 'woocommerce-box-office' ),
		'radio'      => esc_html__( 'Radio buttons', 'woocommerce-box-office' ),
		'checkbox'   => esc_html__( 'Checkbox(es)', 'woocommerce-box-office' ),
	);
...
function wc_box_office_autofill_options() {
	...
	$options = array(
		'billing_first_name' => esc_html__( 'First name', 'woocommerce-box-office' ),
		'billing_last_name'  => esc_html__( 'Last name', 'woocommerce-box-office' ),
		'billing_email'      => esc_html__( 'Email address', 'woocommerce-box-office' ),
		'billing_phone'      => esc_html__( 'Phone', 'woocommerce-box-office' ),
		'billing_company'    => esc_html__( 'Company', 'woocommerce-box-office' ),
		'billing_country'    => esc_html__( 'Country', 'woocommerce-box-office' ),
		'billing_city'       => esc_html__( 'Town / City', 'woocommerce-box-office' ),
		'billing_state'      => esc_html__( 'State', 'woocommerce-box-office' ),
		'billing_postcode'   => esc_html__( 'Postcode / Zip', 'woocommerce-box-office' ),
	);
...
function wcbo_get_ticket_title_prefix() {
	...
	if ( empty( $ticket_prefix ) ) {
		$ticket_prefix = esc_html__( 'Ticket #', 'woocommerce-box-office' );
	}
 ...
 * @param array  $deps     Fallback script dependencies.
 */
function wcbo_register_script( $handle = '', $filename = '', $deps = array() ) {
	// Sanitize filename to allow only alphanumeric, dash, and underscore characters.
	$filename = preg_replace( '/[^a-zA-Z0-9_\-]/', '', $filename );

	$script_url        = WCBO()->assets_url . $filename . '.js';
	$script_asset_path = WCBO()->assets_dir . '/' . $filename . '.asset.php';
	$script_asset      = file_exists( $script_asset_path )
		? require_once $script_asset_path // nosemgrep: audit.php.lang.security.file.inclusion-arg -- already sanitized above.
		: array(
			'dependencies' => $deps,
			'version'      => WOOCOMMERCE_BOX_OFFICE_VERSION,
		);
