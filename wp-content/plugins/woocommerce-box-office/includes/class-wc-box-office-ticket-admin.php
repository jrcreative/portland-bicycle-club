...
class WC_Box_Office_Ticket_Admin {
	...
	public function filter_ticket_product_id() {
		...
		if ( $tickets ) {
			...
			$output        .= '<option value="">' . esc_html__( 'All Ticket Products', 'woocommerce-box-office' ) . '</option>';
			...
			$output .= '<option value="">' . esc_html__( 'All status', 'woocommerce-box-office' ) . '</option>';
			$output .= sprintf( '<option %s value="attended">%s</option>', selected( $checkin_status, 'attended', false ), esc_html__( 'Attended', 'woocommerce-box-office' ) );
			$output .= sprintf( '<option %s value="not-attended">%s</option>', selected( $checkin_status, 'not-attended', false ), esc_html__( 'Not attended', 'woocommerce-box-office' ) );
			$output .= '</select>';
		}
	...
	public function event_ticket_meta_box( $post ) {
		add_meta_box( 'ticket-info', esc_html__( 'Ticket Information', 'woocommerce-box-office' ), array( $this, 'event_ticket_meta_box_content' ), 'event_ticket', 'normal', 'high' );

		if ( function_exists( 'WC_Order_Barcodes' ) ) {
			add_meta_box( 'ticket-barcode', esc_html__( 'Ticket Barcode', 'woocommerce-box-office' ), array( $this, 'display_ticket_barcode_meta_box' ), 'event_ticket', 'side', 'default' );
		}
	}
	...
	public function event_ticket_email_meta_box ( $post ) {
		add_meta_box( 'email-info', esc_html__( 'Email Information', 'woocommerce-box-office' ), array( $this, 'event_ticket_email_meta_box_content' ), 'event_ticket_email', 'normal', 'high' );
		add_meta_box( 'email-content', esc_html__( 'Email Content', 'woocommerce-box-office' ), array( $this, 'event_ticket_email_content_meta_box_content' ), 'event_ticket_email', 'normal', 'high' );
		add_meta_box( 'email-log', esc_html__( 'Email Log', 'woocommerce-box-office' ), array( $this, 'event_ticket_email_log_meta_box_content' ), 'event_ticket_email', 'normal', 'high' );
	...
	public function manage_ticket_columns( $columns = array() ) {
		...
		// Add custom columns.
		$columns['ticket']     = esc_html__( 'Ticket', 'woocommerce-box-office' );
		$columns['order']      = esc_html__( 'Order', 'woocommerce-box-office' );
		$columns['checked-in'] = esc_html__( 'Checked-in yet?', 'woocommerce-box-office' );
		$columns['opted-out']  = esc_html__( 'Opted-out of public listing', 'woocommerce-box-office' );
		$columns['date']       = esc_html__( 'Date', 'woocommerce-box-office' );
	...
	public function modify_bulk_actions( $actions ) {
		unset( $actions['edit'] );
		$actions['mark_attended']       = esc_html__( 'Mark as attended', 'woocommerce-box-office' );
		$actions['mark_not_checked_in'] = esc_html__( 'Mark as not checked-in yet', 'woocommerce-box-office' );
		$actions['mark_opted_out']      = esc_html__( 'Mark as opted-out of public listing', 'woocommerce-box-office' );
		$actions['mark_opted_in']       = esc_html__( 'Mark as opted-in for public listing', 'woocommerce-box-office' );
	...
	public function add_create_ticket_page() {
		$create_ticket_page = add_submenu_page( 'edit.php?post_type=event_ticket', esc_html__( 'Create Ticket', 'woocommerce-box-office' ), esc_html__( 'Create Ticket', 'woocommerce-box-office' ), 'manage_woocommerce', 'create_ticket', array( $this, 'create_ticket_page' ) );
	}
