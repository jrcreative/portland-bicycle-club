...
class WC_Box_Office_Privacy extends WC_Abstract_Privacy {
	...
	public function register_erasers_exporters() {
		$this->name = __( 'Box Office', 'woocommerce-box-office' );

		$this->add_exporter( 'woocommerce-box-office-data', esc_html__( 'WooCommerce Box Office Data', 'woocommerce-box-office' ), array( $this, 'ticket_data_exporter' ) );
		$this->add_eraser( 'woocommerce-box-office-data', esc_html__( 'WooCommerce Box Office Data', 'woocommerce-box-office' ), array( $this, 'ticket_data_eraser' ) );
	}
		...
		if ( 0 < count( $tickets ) ) {
			foreach ( $tickets as $ticket ) {
				$data_to_export[] = array(
					...
					'group_label' => esc_html__( 'Tickets', 'woocommerce-box-office' ),
					'item_id'     => 'ticket-' . $ticket->ID,
					'data'        => array(
						array(
							'name'  => esc_html__( 'Ticket ID', 'woocommerce-box-office' ),
							'value' => $ticket->ID,
						),
						array(
							'name'  => esc_html__( 'Ticket data', 'woocommerce-box-office' ),
	...
	public function ticket_data_eraser( $email_address, $page ) {
		...
		if ( $items_removed ) {
			$messages[] = esc_html__( 'Box office personal data erased.', 'woocommerce-box-office' );
		}
