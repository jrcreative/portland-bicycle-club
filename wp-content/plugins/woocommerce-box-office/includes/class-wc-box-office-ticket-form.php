...
class WC_Box_Office_Ticket_Form {
	...
	private function _multiple_tickets_js() {
		wcbo_register_script( 'wc-box-office-multiple-tickets', 'multiple-tickets' );
		wp_enqueue_script( 'wc-box-office-multiple-tickets' );

		$add_to_cart_singular = get_option( 'box_office_add_to_cart_singular', '' );
		if ( empty( $add_to_cart_singular ) ) {
			$add_to_cart_singular = esc_html__( 'Buy Ticket Now', 'woocommerce-box-office' );
		}

		$add_to_cart_plural = get_option( 'box_office_add_to_cart_plural', '' );
		if ( empty( $add_to_cart_plural ) ) {
			$add_to_cart_plural = esc_html__( 'Buy Tickets Now', 'woocommerce-box-office' );
		}
		...
		$params = apply_filters( 'woocommerce_box_office_ticket_form_params', array(
			...
			'i18n_fields_required'      => esc_html__( 'Fields are required', 'woocommerce-box-office' ),
	...
	private function _validate_field( $key, $value ) {
		...
		if ( $this->_field_prop_is( $key, 'required', 'yes' ) && empty( $value ) ) {
			throw new Exception(
				sprintf(
					esc_html__( '%s is required field and can not be empty.', 'woocommerce-box-office' ),
					esc_html( $this->_field_prop_val( $key, 'label' ) )
				)
			);
		}
		...
		if ( ! empty( $value ) ) {
			switch ( $type ) {
				case 'email':
					if ( ! is_email( $value ) ) {
						throw new Exception( esc_html__( 'Invalid email is provided.', 'woocommerce-box-office' ) );
					}
					break;
				case 'url':
					...
					if ( false === filter_var( $value, FILTER_VALIDATE_URL ) ) {
						throw new Exception( esc_html__( 'Invalid URL is provided', 'woocommerce-box-office' ) );
					}
					break;
				case 'select':
				case 'radio':
				case 'checkbox':
					...
					foreach ( $value as $val ) {
						if ( ! in_array( $val, $options ) ) {
							throw new Exception( sprintf( esc_html__( 'Invalid value for field %s', 'woocommerce-box-office' ), esc_html( $this->_field_prop_val( $key, 'label' ) ) ) );
						}
					}
					break;
			}
	...
	public function get_clean_data() {
		if ( ! $this->_validated ) {
			_doing_it_wrong( __FUNCTION__, esc_html( sprintf( esc_html__( 'Call %s::validate first', 'woocommerce-box-office' ), __CLASS__ ) ), esc_html( WCBO()->_version ) );
		}
