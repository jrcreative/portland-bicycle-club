...
class WC_Box_Office_Ticket_Shortcode {
	...
	public function my_ticket( $params = array() ) {
		...
		if ( empty( $ticket->fields ) ) {
			return esc_html__( 'No field defined for this product.', 'woocommerce-box-office' );
		}
		...
		$description = sprintf(
			...
			'pending' === $ticket->status ? ' &mdash; ' . esc_html__( 'Pending', 'woocommerce-box-office' ) : ''
		);
	...
	public function user_ticket_list( $params = array() ) {
		...
		$attributes = shortcode_atts(
			array(
				...
				'description'    => __( 'View or edit each of your purchased tickets using the links below.', 'woocommerce-box-office' ),
				'enable_filters' => true,
			),
			$params
		);
		...
		wc_get_template(
			...
			array(
				...
				'enable_filters' => $attributes['enable_filters'],
			),
	 ...
	 */
	public function private_content( $atts = array(), $content = '' ) {
		...
		try {
			...
			if ( empty( $_GET['token'] ) ) {
				throw new Exception( esc_html__( 'The content on this page is private.', 'woocommerce-box-office' ), 400 );
			}
	...
	public function maybe_send_private_content_link() {
		...
		if ( ! is_email( $email ) ) {
			...
			$this->_notice_link_retrieval      = esc_html__( 'The e-mail address you have entered does not seem to be valid.', 'woocommerce-box-office' );
			return;
		}
