...
class WC_Box_Office_Tools {
	...
	public function add_menu_item() {
		add_submenu_page( 'edit.php?post_type=event_ticket', esc_html__( 'Ticket Tools', 'woocommerce-box-office' ), esc_html__( 'Tools', 'woocommerce-box-office' ), 'manage_woocommerce', 'ticket_tools', array( $this, 'tools_page' ) );
	}
	...
	public function add_tools_link( $links ) {
		$settings_link = '<a href="edit.php?post_type=event_ticket&page=ticket_tools">' . esc_html__( 'Tools', 'woocommerce-box-office' ) . '</a>';
	...
	public function show_test_email() {
		...
		if ( empty( $ticket_ids ) ) {
			$response = '<p><strong><em>' . esc_html__( 'No purchased tickets found for the selected products.', 'woocommerce-box-office' ) . '</em></strong></p>';
		} else {
			...
			if ( ! empty( $subject ) ) {
				...
				$subject = '<em>' . esc_html__( '(no subject)', 'woocommerce-box-office' ) . '</em>';
			}
	...
	public function export_tickets() {
		...
		$columns = array(
			'ticket_id'     => esc_html__( 'Ticket ID', 'woocommerce-box-office' ),
			'ticket_status' => esc_html__( 'Ticket status', 'woocommerce-box-office' ),
			'sku'           => esc_html__( 'SKU', 'woocommerce-box-office' ),
			'ticket_name'   => esc_html__( 'Ticket', 'woocommerce-box-office' ),
			'ticket_url'    => esc_html__( 'Ticket URL', 'woocommerce-box-office' ),
			'purchase_date' => esc_html__( 'Purchase date', 'woocommerce-box-office' ),
			'order_id'      => esc_html__( 'Order ID', 'woocommerce-box-office' ),
			'order_status'  => esc_html__( 'Order status', 'woocommerce-box-office' ),
			'coupon_code'   => esc_html__( 'Coupon Code', 'woocommerce-box-office' ),
			'user_id'       => esc_html__( 'User ID', 'woocommerce-box-office' ),
			'is_checked_in' => esc_html__( 'Attended', 'woocommerce-box-office' ),
			'is_opted_out'  => esc_html__( 'Opted-out of public listing', 'woocommerce-box-office' ),
		);
		...
		ob_start();
		$export = fopen( 'php://output', 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_fopen
		...
		while ( $tickets = get_posts(
			...
			foreach ( $tickets as $ticket ) {
				...
				if ( ! $order || ( $order && '0000-00-00 00:00:00' === $order->get_date_created() ) ) {
					$purchase_time = esc_html__( 'N/A', 'woocommerce-box-office' );
				} else {
					$purchase_time = get_the_time( esc_html__( 'Y/m/d g:i:s A', 'woocommerce-box-office' ), esc_html( $order_id ) );
				}
		...
		fclose( $export ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_fclose, WordPress.WP.AlternativeFunctions.file_system_operations_fclose
		$export = ob_get_clean();
	...
	public function email_tickets() {
		...
		try {
			$required_fields = array(
				'product_id'              => esc_html__( 'Ticket product is required.', 'woocommerce-box-office' ),
				'email_subject'           => esc_html__( 'Empty email subject.', 'woocommerce-box-office' ),
				'email_body'              => esc_html__( 'Empty email body.', 'woocommerce-box-office' ),
				'tools_send_emails_nonce' => esc_html__( 'Missing security nonce for this request. Please try again.', 'woocommerce-box-office' ),
			);
			...
			if ( ! wp_verify_nonce( $_POST['tools_send_emails_nonce'], 'woocommerce_box_office_tools_send_emails' ) ) {
				throw new Exception( esc_html__( 'Invalid security nonce for this request. Please try again.', 'woocommerce-box-office' ) );
			}
