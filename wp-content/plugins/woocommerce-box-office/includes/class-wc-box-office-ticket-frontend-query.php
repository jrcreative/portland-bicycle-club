...
class WC_Box_Office_Ticket_Frontend_Query extends WC_Query {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_get_query_vars', array( $this, 'add_my_tickets_woocommerce_query_vars' ) );
		add_filter( 'woocommerce_endpoint_my-tickets_title', array( $this, 'change_endpoint_my_tickets_title' ) );
	...
	/**
	 * Add a query var for my-tickets.
	 *
	 * @since 1.2.8
	 *
	 ...
	 */
	public function add_my_tickets_woocommerce_query_vars( $vars ) {
		$vars['my-tickets'] = 'my-tickets';

		return $vars;
	}

	/**
	 * Update the title for the my-tickets endpoint.
	 *
	 * @since 1.2.8
	 *
	 * @global $wp WP The WordPress environment.
	 *
	 * @param string $title Title.
	 *
	 * @return string Altered title
	 */
	public function change_endpoint_my_tickets_title( $title ) {
		global $wp;

		$page = (int) $wp->query_vars['my-tickets'];

		if ( ! empty( $page ) ) {
			return sprintf(
				/* translators: %d: page number */
				esc_html__( 'My Tickets (page %d)', 'woocommerce-box-office' ),
				$page
			);
		}

		return esc_html__( 'My Tickets', 'woocommerce-box-office' );
	}
	 ...
	 * Get endpoint title.
	 *
	 * @global $wp The WordPress environment.
	 *
	...
	public function add_menu_items( $menu_items ) {
		...
		if ( isset( $menu_items['orders'] ) ) {
			...
			foreach ( $menu_items as $key => $menu ) {
				...
				if ( 'orders' === $key ) {
					$new_menu_items['my-tickets'] = esc_html__( 'My Tickets', 'woocommerce-box-office' );
				}
			}
			$menu_items = $new_menu_items;
		} else {
			$menu_items['my-tickets'] = esc_html__( 'My Tickets', 'woocommerce-box-office' );
		}
