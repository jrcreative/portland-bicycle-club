	...
	public function maybe_process_updated_ticket() {
		...
		try {
			if ( empty( $_POST['_wpnonce'] ) ) {
				throw new Exception( esc_html__( 'Missing nonce to update the ticket.', 'woocommerce-box-office' ) );
			}

			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'woocommerce-box-office_update_ticket' ) ) {
				throw new Exception( esc_html__( 'We were unable to update your ticket, please try again.', 'woocommerce-box-office' ) );
			}

			$ticket = wc_box_office_get_ticket_by_token( sanitize_text_field( wp_unslash( $_GET['token'] ?? '' ) ) );
			if ( ! $ticket ) {
				throw new Exception( esc_html__( 'Invalid ticket.', 'woocommerce-box-office' ) );
			}

			if ( ! is_ticket_editable( $ticket ) ) {
				throw new Exception( esc_html__( 'Ticket is not editable.', 'woocommerce-box-office' ) );
			}
			...
			wc_add_notice( esc_html__( 'Ticket updated.', 'woocommerce-box-office' ), 'success' );
