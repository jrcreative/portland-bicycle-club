...
class Webhooks {
	...
	private function get_payload( $api, $resource_id, $webhook_id ) {
		...
		try {
			...
			wp_set_current_user( $webhook->get_user_id() ); // @phpcs:ignore Generic.PHP.ForbiddenFunctions.Discouraged
			...
			wp_set_current_user( $old_user ); // @phpcs:ignore Generic.PHP.ForbiddenFunctions.Discouraged
