...
class User_Memberships extends Controller {
	...
	/**
	 * Validated directory block settings for the current request.
	 * @see User_Memberships::get_directory_members()
	 *
	 * @since 1.27.3
	 */
	protected array $directory_block_settings = [];
	...
	public function register_routes() {
		...
		// endpoint: 'wc/v<n>/memberships/members/directory' (similar to GET /members above, but specific to directory blocks)
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/directory', [
			[
				'methods'             => \WP_REST_Server::READABLE,
				/** @see User_Memberships::get_directory_members() */
				'callback'            => [ $this, 'get_directory_members' ],
				/** @see User_Memberships::get_directory_members_permissions_check() */
				'permission_callback' => [ $this, 'get_directory_members_permissions_check' ],
				'args'                => $this->get_directory_collection_params(),
			],
		], true );
	...
	private function get_request_membership_plan( $request ) {
		...
		if ( $plan_id ) {
			...
			if ( ! $plan instanceof \WC_Memberships_Membership_Plan ) {
				...
				throw new \WC_REST_Exception(esc_html( "woocommerce_rest_{$this->post_type}_plan_not_found" ), sprintf( esc_html__( 'Membership Plan%s invalid or not found.', 'woocommerce-memberships' ), is_string( $plan_id ) || is_numeric( $plan_id ) ? esc_html( ' '.$plan_id ) : '' ), 404 );
			}
		}
	...
	private function get_request_customer_user( $request ) {
		...
		if ( $user_id ) {
			...
			if ( ! $user ) {
				/* translators: Placeholder: %s - customer identifier (may be empty) */
				throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_customer_id" ), sprintf( esc_html__( 'Customer%s invalid or not found.', 'woocommerce-memberships' ), is_string( $user_id ) || is_numeric( $user_id ) ? esc_html( ' '.$user_id ) : '' ), 404 );
			}
		}
	...
	private function set_user_membership_data( $user_membership, $request ) {
		...
		foreach ( $dates as $date_id ) {
			...
			if ( isset( $request[ $date_id ] ) ) {
				...
				if ( is_string( $date_value) && is_callable( [ $user_membership, $set_date ] ) ) {
					...
					throw new \WC_REST_Exception( esc_html( "woocommerce_rest_invalid_{$this->post_type}_{$date_id}" ), sprintf( esc_html__( 'Invalid %s date value.', 'woocommerce-memberships' ), esc_html( $date_id ) ), 400 );
				}
			}
		}
		...
		if ( $product_id = isset( $request['product_id'] ) ? $request['product_id'] : null ) {
			...
			if ( $product instanceof \WC_Product ) {
				...
				throw new \WC_REST_Exception( esc_html( "woocommerce_rest_invalid_{$this->post_type}_product_id" ), sprintf (esc_html__( 'Product%s invalid or not found.', 'woocommerce-memberships' ), is_numeric( $product ) ? esc_html( ' '.$product ) : '' ), 404 );
			}
		}
		...
		if ( $order_id = isset( $request['order_id'] ) ? $request['order_id'] : null ) {
			...
			if ( $order instanceof \WC_Order || $order instanceof \WC_Order_Refund ) {
				...
				throw new \WC_REST_Exception( esc_html( "woocommerce_rest_invalid_{$this->post_type}_order_id" ), sprintf( esc_html__( 'Order %s invalid or not found.', 'woocommerce-memberships' ), is_numeric( $order ) ? esc_html( $order ) : '' ), 404 );
			}
		}
		...
		if ( isset( $request['status'] ) ) {
			...
			if ( $status && wc_memberships()->get_user_memberships_instance()->is_user_membership_status( $status ) ) {
				...
				throw new \WC_REST_Exception( esc_html( "woocommerce_rest_invalid_{$this->post_type}_status" ), sprintf( esc_html__( 'Invalid user membership status: "%s".', 'woocommerce-memberships' ), is_string( $status ) ? esc_html( $status ) : '' ), 400 );
			}
		}
		...
		if ( ! empty( $request['profile_fields'] ) ) {
			...
			if ( ! is_array( $request_profile_fields ) ) {
				throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_profile_fields" ), esc_html__( 'Invalid profile fields.', 'woocommerce-memberships' ), 400 );
			}
			...
			foreach ( $request_profile_fields as $key => $request_profile_field ) {
				...
				if ( empty( $slug ) ) {
					throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_profile_field" ), sprintf(
						/* translators: Placeholders: %d - array item index */
						esc_html__( 'The profile field slug in "profile_fields[%d][slug]" is required.', 'woocommerce-memberships' ),
						esc_html( $key )
					), 400 );
				}
				...
				if ( ! isset( $request_profile_field['value'] ) ) {
					throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_profile_field" ), sprintf(
						/* translators: Placeholders: %d - array item index */
						esc_html__( 'The profile field value in "profile_fields[%d][value]" is required.', 'woocommerce-memberships' ),
						esc_html( $key )
					), 400 );
				}
				...
				try {
					...
					if ( ! $field_definition instanceof Profile_Fields\Profile_Field_Definition ) {
						...
						throw new Framework\SV_WC_Plugin_Exception( sprintf( esc_html__( 'No profile field definition found for "%s".', 'woocommerce-memberships' ), esc_html( $slug ) ), 404 );
					}
					...
					if ( $field_definition->is_type( Profile_Fields::TYPE_FILE ) && ! wp_get_attachment_url( (int) $value ) ) {
						...
						throw new Framework\SV_WC_Plugin_Exception( sprintf( esc_html__( 'The value "%1$s" for the "%2$s" profile field is not a valid attachment ID.', 'woocommerce-memberships' ), esc_html( $value ), esc_html( $slug ) ) );
					}
					...
					throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_profile_field" ), sprintf(
						...
						esc_html__( 'The "profile_fields[%1$s]" data is invalid. %2$s', 'woocommerce-memberships' ),
						esc_html( $key ),
		...
		if ( ! empty( $request['meta_data'] ) ) {
			...
			if ( ! is_array( $meta_data ) ) {
				throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_meta_data" ), esc_html__( 'Invalid meta data.', 'woocommerce-memberships' ), 400 );
			}
			...
			foreach ( $meta_data as $meta ) {
				...
				if ( ! is_string( $meta_key ) || '' === trim( $meta_key ) ) {
					throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_meta_data" ), esc_html__( 'Invalid meta data key.', 'woocommerce-memberships' ), 404 );
				}
				...
				if ( null !== $meta_value && ! is_bool( $meta_value ) && ! is_string( $meta_value ) && ! is_numeric( $meta_value ) && ! is_array( $meta_value ) ) {
					throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_invalid_meta_data" ), esc_html__( 'Invalid meta data value.', 'woocommerce-memberships' ), 404 );
				}
		...
		if ( ! $user_membership instanceof \WC_Memberships_User_Membership ) {
			throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$this->post_type}_error" ), sprintf( esc_html__( 'An error occurred while setting properties for the user membership %s.', 'woocommerce-memberships' ), esc_html( $user_membership_id ) ), 500 );
		}
	...
	/**
	 * Gets collection parameters specific to directory blocks.
	 *
	 * @since 1.27.3
	 *
	 * @return array associative array
	 */
	public function get_directory_collection_params() {

		$params = $this->get_collection_params();

		// Add directory-specific parameters
		$params['page_id'] = [
			'description'       => __( 'Page ID containing the directory block.', 'woocommerce-memberships' ),
			'type'              => 'integer',
			'required'          => true,
		...
		$params['block_instance_id'] = [
			'description'       => __( 'Block instance identifier for settings validation.', 'woocommerce-memberships' ),
			'type'              => 'string',
		...
		/**
		 * Filters the directory collection params for REST API queries.
		 *
		 * @since 1.27.3
		 *
		 * @param array $params associative array
		 */
		return (array) apply_filters( 'wc_memberships_rest_api_directory_collection_params', $params );
	}
	...
	/**
	 * Gets members for directory blocks with privacy controls.
	 *
	 * @since 1.27.3
	 *
	 * @param \WP_REST_Request $request request object
	 ...
	 */
	public function get_directory_members( $request ) {

		$validator = new Directory_Block_Validator();

		// Get block settings with validation (page access already validated in permission callback)
		$block_settings = $validator->get_validated_block_settings( $request );
		...
		// Call existing get_items method to reuse all the membership query logic
	...
	/**
	 * Adds customer data to membership data for directory blocks.
	 *
	 * @since 1.27.3
	 *
	 * @param array|mixed $data membership data
	 * @param \WC_Memberships_User_Membership|null|mixed $user_membership membership object
	 * @param \WP_REST_Request|null|mixed $request request object
	 * @return array|mixed modified membership data
	 */
	public function add_directory_customer_data( $data, $user_membership, $request ) {

		// Only add customer data if we have stored block settings (indicating this is a directory request)
		if ( empty( $this->directory_block_settings ) || ! $user_membership instanceof \WC_Memberships_User_Membership ) {
			return $data;
		}
		...
		try {
			...
			$block_settings = $this->directory_block_settings;

			// Always include name fields for directory display
			$filtered_customer_data = [
				'first_name' => $customer->get_first_name(),
				'last_name'  => $customer->get_last_name(),
			];

			// Only include fields that are allowed by validated block settings
			if ( ! empty( $block_settings['showEmail'] ) ) {
				$filtered_customer_data['user_email'] = $customer->get_email();
			}

			if ( ! empty( $block_settings['showPhone'] ) ) {
				$filtered_customer_data['phone'] = $customer->get_billing_phone();
			}

			if ( ! empty( $block_settings['showAddress'] ) ) {
				$filtered_customer_data['address'] = $customer->get_billing_address();
			}

			if ( ! empty( $block_settings['showBio'] ) ) {
				$filtered_customer_data['bio'] = $user ? $user->get( 'description' ) : '';
			}

			if ( ! empty( $block_settings['avatar'] ) ) {
				$filtered_customer_data['avatar'] = get_avatar_url( $data['customer_id'] ) ?: null;
			}

			$data['customer_data'] = $filtered_customer_data;
	...
	public function get_users_for_search( string $search_string = '' ) : array {
		...
		return array_unique( array_column( $wpdb->get_results( $users_query ), 'user_id' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
