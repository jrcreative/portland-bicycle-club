...
class WC_Memberships_CSV_Import_User_Memberships extends \WC_Memberships_Job_Handler {
	...
	private function get_file_upload_error_message( $error_code = null ) : string {
		...
		switch ( $error_code ) {
			case 1 :
			case 2 :
				return esc_html__( 'The file uploaded exceeds the maximum file size allowed.', 'woocommerce-memberships' );
			case 3 :
				return esc_html__( 'The file was only partially uploaded. Please try again.', 'woocommerce-memberships' );
			case 4 :
				return esc_html__( 'No file was uploaded.', 'woocommerce-memberships' );
			case 6 :
				return esc_html__( 'Missing a temporary folder to store the file. Please contact your host.', 'woocommerce-memberships' );
			case 7 :
				return esc_html__( 'Failed to write file to disk. Perhaps a permissions error, please contact your host.', 'woocommerce-memberships' );
			case 8 :
				return esc_html__( 'A PHP Extension stopped the file upload. Please contact your host.', 'woocommerce-memberships' );
			default :
				return esc_html__( 'File upload error.', 'woocommerce-memberships' );
		}
	}
	...
	public function create_job( $attrs ) {
		...
		if ( isset( $attrs['file']['error'] ) && is_numeric( $attrs['file']['error'] ) && $attrs['file']['error'] > 0 ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html( $this->get_file_upload_error_message( $attrs['file']['error'] ) ) );
		}

		// file is too big
		if ( isset( $attrs['file']['size'] ) && is_numeric( $attrs['file']['size'] ) > wc_let_to_num( ini_get( 'post_max_size' ) ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html( $this->get_file_upload_error_message( 1 ) ) );
		} else {
		...
		if ( '' === $file_name ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( esc_html( $this->get_file_upload_error_message() ) );
		} else {
		...
		if ( ! wp_mkdir_p( $imports_dir ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html( $write_error_message ) );
		}
	...
	public function process_job( $job, $items_per_batch = 3 ) {
		...
		if ( empty( $job->file_name ) || ! is_string( $job->file_name ) || empty( $job->file_path ) || ! is_string( $job->file_path ) || ! is_readable( $job->file_path ) ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( esc_html( $read_error_message ) );
		}
