		...
		if ( is_numeric( $date ) ) {
			...
			if ( is_string( $source_format ) && '' !== trim( $source_format ) ) {
				...
				try {
					...
					trigger_error( esc_html( sprintf( 'Failed to parse date "%1$s" with format "%2$s": %3$s', $date, $source_format, $e->getMessage() ) ), E_USER_WARNING );
		...
		if ( ! empty( $source_date ) ) {
			...
			if ( 'UTC' === $source_timezone ) {
				...
				try {
					...
					trigger_error( esc_html( sprintf( 'Failed to parse date "%1$s": %2$s', $date, $e->getMessage() ) ), E_USER_WARNING );
				...
				try {
					...
					// in case of DateTime errors, just return the date as is but issue an error
					trigger_error( esc_html( sprintf( 'Failed to parse date "%1$s" to get timezone offset: %2$s.', $date, $e->getMessage() ) ), E_USER_WARNING );
