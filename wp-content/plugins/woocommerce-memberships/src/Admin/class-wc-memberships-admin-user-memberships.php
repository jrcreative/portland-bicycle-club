...
class WC_Memberships_Admin_User_Memberships {
		...
		switch ( $column ) {
			...
			case 'plan':
				...
				if ( $user_membership ) {
					...
					if ( $plan = $user_membership->get_plan() ) {
						echo '<a href="' . esc_url( get_edit_post_link( $user_membership->get_plan_id() ) ) . '">' . esc_html( $plan->get_formatted_name() ) . '</a>';
					} else {
						echo '-';
					}
				}
			...
			case 'status':
				...
				if ( 'paused' === $status && ( $paused_date = $user_membership->get_local_paused_date( 'timestamp' ) ) ) {
					...
					printf( esc_html__( 'Paused since %s', 'woocommerce-memberships' ), esc_html( date_i18n( wc_date_format(), $paused_date ) . ' ' . date_i18n( wc_time_format(), $paused_date ) ) );
					...
					printf( esc_html__( 'Cancelled on %s', 'woocommerce-memberships'), esc_html( date_i18n( wc_date_format(), $cancelled_date ) . ' ' . date_i18n( wc_time_format(), $cancelled_date ) ) );
			...
			case 'member_since':
				...
				if ( $user_membership ) {
					...
					$date = date_i18n( $date_format, (int) $since_time );
					$time = date_i18n( $time_format, (int) $since_time );

					printf( '<span class="member-since-date">%1$s %2$s</span>', esc_html( $date ), esc_html( $time ) );
					...
					if ( $order_id && $order ) {
						...
						printf( '<span class="member-since-order"><small>' . esc_html__( 'Order: %s', 'woocommerce-memberships' ) . '</small></span>', '<a href="' . esc_url( get_edit_post_link( $order_id ) ) . '">#' . esc_html( $order->get_order_number() ) . '</a>' );
					}
				}
			...
			case 'expires':
				...
				if ( $user_membership ) {
					...
					if ( ! empty( $end_time ) && is_numeric( $end_time ) ) {
						$date = date_i18n( $date_format, (int) $end_time );
						$time = date_i18n( $time_format, (int) $end_time );
					} else {
						$date = __( 'Never', 'woocommerce-memberships' );
						$time = '';
					}
			...
			case 'last_login' :
				...
				echo esc_html( $user_membership->get_last_active_since() ?: '&mdash;' );
					...
					if ( Profile_Fields::is_profile_field_slug( $profile_field_slug ) ) {
						...
						if( $profile_field = $user_membership->get_profile_field( $profile_field_slug ) ) {
							...
							if ( $profile_field->get_definition()->is_type( Profile_Fields::TYPE_FILE ) ) {
								...
								echo $media_library_url ? sprintf( '<a href="%s">%s</a>', esc_url( $media_library_url ), esc_html( basename( get_attached_file( $profile_field->get_value() ) ) ) ) : $no_profile_field_value; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							} else {
								echo $profile_field->get_value() ? esc_html( $profile_field->get_formatted_value() ) : $no_profile_field_value; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							}

						} else {

							echo $no_profile_field_value; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
					}
				}
	...
	public function request_query( $vars ) {
		...
		if ( 'wc_user_membership' === $typenow ) {
			...
			if ( isset( $_GET['post_parent'] ) ) {
				$vars['post_parent'] = sanitize_text_field( wp_unslash( $_GET['post_parent'] ) );
			}
	...
	private function parse_profile_fields_filters() {
		...
		if ( ! empty( $_GET['profile_fields'] ) ) {
			...
			$filter = json_decode( wp_unslash( $_GET['profile_fields'] ), false ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( isset( $wp_query->query['s'] ) ) {
			...
			if ( ! empty( $keyword ) ) {
				...
				if ( ! empty( $_GET['post_status'] ) ) {
					$where_post_status = $wpdb->prepare( "$wpdb->posts.post_status = %s", sanitize_text_field( wp_unslash( $_GET['post_status'] ) ) );
				} else {
	...
	public function restrict_user_memberships() {
		...
		$selected_plan    = isset( $_GET['post_parent'] ) ? sanitize_text_field( wp_unslash( $_GET['post_parent'] ) ) : null;

		$statuses        = wc_memberships_get_user_membership_statuses();
		$selected_status = isset( $_GET['post_status'] ) ? sanitize_text_field( wp_unslash( $_GET['post_status'] ) ) : null;
		...
		$selected_expiry_term = isset( $_GET['expires'] ) ? sanitize_text_field( wp_unslash( $_GET['expires'] ) ) : null;
		...
		<div id="wc-memberships-user-memberships-advanced-filters">
			...
			<div class="wc-memberships-user-memberships-advanced-filters-header">
				<h3><?php printf(
					...
					$filter_operand // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				); ?></h3>
			</div>
	...
	private function output_profile_fields_filter_row( $row_index = null, $row_data = null ) {
		...
		?>
		<li id="wc-memberships-user-membership-advanced-filter<?php echo esc_attr( $row_index ); ?>" class="wc-memberships-user-membership-advanced-filters-list-item">
	...
	public function pause_membership() {
		...
		wp_safe_redirect( add_query_arg( array('paused' => 1, 'ids' => sanitize_text_field( wp_unslash( $_REQUEST['post'] ) ) ), $this->get_sendback_url() ) );
		exit();
	}
	...
	public function resume_membership() {
		...
		wp_safe_redirect( add_query_arg( array('resumed' => 1, 'ids' => sanitize_text_field( wp_unslash( $_REQUEST['post'] ) ) ), $this->get_sendback_url() ) );
		exit();
	}
	...
	public function cancel_membership() {
		...
		wp_safe_redirect( add_query_arg( array('cancelled' => 1, 'ids' => sanitize_text_field( wp_unslash( $_REQUEST['post'] ) ) ), $this->get_sendback_url() ) );
		exit();
	}
	...
	public function admin_notices() {
		...
		if ( 'edit.php' === $pagenow ) {
			...
			if ( isset( $_REQUEST['paused'] ) && (int) $_REQUEST['paused'] ) {
				$numberPaused = (int) wp_unslash( $_REQUEST['paused'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				/* translators: Placeholder: %s - number of memberships paused */
				$message = sprintf( _n( 'User membership paused.', '%s user memberships paused.', $numberPaused ), number_format_i18n( $numberPaused ), 'woocommerce-memberships' );
			}

			if ( isset( $_REQUEST['cancelled'] ) && (int) $_REQUEST['cancelled'] ) {
				$numberCancelled = (int) wp_unslash( $_REQUEST['cancelled'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				/* translators: Placeholder: %s - number of memberships cancelled */
				$message = sprintf( _n( 'User membership cancelled.', '%s user memberships cancelled.', $numberCancelled ), number_format_i18n( $numberCancelled ), 'woocommerce-memberships' );
			}

			if ( isset( $_REQUEST['resumed'] ) && (int) $_REQUEST['resumed'] ) {
				$numberResumed = (int) wp_unslash( $_REQUEST['resumed'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				/* translators: Placeholder: %s - number of memberships resumed */
				$message = sprintf( _n( 'User membership resumed.', '%s user memberships resumed.', $numberResumed ), number_format_i18n( $numberResumed ), 'woocommerce-memberships' );
			}
	...
	public function maybe_prevent_adding_user_membership() {
		...
		if ( 'post-new.php' === $pagenow ) {
			...
			// get user details
			$user_id = isset( $_GET['user'] ) ? sanitize_text_field( wp_unslash( $_GET['user'] ) ) : null;
