...
class Setup_Wizard extends Framework\Admin\Setup_Wizard {
	...
	protected function render_settings_step() {
		...
		<div class="restriction-mode">
			...
			<div class="redirect-page" style="margin-bottom: 18px; <?php if ( 'redirect' !== $default_restriction_mode ) { echo ' display:none; '; } ?>">
				...
				echo str_replace( 'id=', ' style="width:100%;" data-placeholder="' . esc_attr__( 'Select a page&hellip;', 'woocommerce-memberships' ) . '" class="wc-enhanced-select" id=', wp_dropdown_pages( $args ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	protected function save_settings_preferences() {
		...
		if ( ! empty( $_POST['restriction_mode'] ) ) {
			...
			$this->get_plugin()->get_restrictions_instance()->set_restriction_mode( sanitize_text_field( wp_unslash( $_POST['restriction_mode'] ) ) );

			if ( 'redirect' === $_POST['restriction_mode'] && isset( $_POST['redirect_page_id'] ) ) {
				$this->get_plugin()->get_restrictions_instance()->set_restricted_content_redirect_page_id( absint( trim( wp_unslash( $_POST['redirect_page_id'] ) ) ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			}
		}
				...
				<select
					...
					<?php foreach ( $product_ids as $product_id ) : ?>
						<?php if ( $product = wc_get_product( $product_id ) ) : ?>
							<option value="<?php echo esc_attr( $product_id ); ?>" selected><?php echo esc_html( strip_tags( $product->get_formatted_name() ) ); ?></option>
						<?php endif; ?>
					<?php endforeach; ?>
				</select>
			</div>
	...
	protected function save_access_preferences() {
		...
		if ( $plan = $this->get_my_first_membership_plan() ) {
			...
			if ( ! isset( $_POST['membership_plan_name'] ) || '' === trim( wp_unslash( $_POST['membership_plan_name'] ) ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'You must enter a plan name.', 'woocommerce-memberships' ) );
			}

			$name = sanitize_text_field( wp_unslash( $_POST['membership_plan_name'] ) );
			...
			if ( isset( $_POST['access_method'] ) ) {
				...
				$plan->set_access_method( sanitize_text_field( wp_unslash( $_POST['access_method'] ) ) );

				if ( 'purchase' === $_POST['access_method'] && isset( $_POST['product_ids'] ) ) {
					$plan->set_product_ids( (array) $_POST['product_ids'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
				} else {
			...
			if ( isset( $_POST['access_length_type'] ) && is_string( $_POST['access_length_type'] ) ) {
				...
				switch ( $_POST['access_length_type'] ) {
					...
					case 'specific' :
						...
						$access_length_amount = isset( $_POST['access_length_amount'] ) ? sanitize_text_field( wp_unslash( $_POST['access_length_amount'] ) ) : '';
						$access_length_period = isset( $_POST['access_length_period'] ) ? sanitize_text_field( wp_unslash( $_POST['access_length_period'] ) ) : '';
					...
					case 'fixed' :
						...
						$access_start_date = isset( $_POST['access_start_date'] ) ? sanitize_text_field( wp_unslash( $_POST['access_start_date'] ) ) : '';
						$access_end_date   = isset( $_POST['access_end_date'] ) ? sanitize_text_field( wp_unslash( $_POST['access_end_date'] ) ) : '';
						...
						<td colspan="2" style="width: 100%;">
							<div class="form-row sv-wc-plugin-admin-setup-control purchasing-discount-type-amount">
								...
								<select
									...
									<option value="amount"><?php echo esc_html( get_woocommerce_currency_symbol() ); ?></option>
								</select>
							</div>
						</td>
					</tr>
	...
	protected function save_member_perks_preferences() {
		...
		if ( $plan = $this->get_my_first_membership_plan() ) {
			...
			if ( isset( $_POST['restrict_content'], $_POST['content_restriction_rule_content_type'] ) && 'yes' === $_POST['restrict_content'] ) {
				...
				$content_type = $this->get_membership_plan_rule_content_type_group( sanitize_text_field( wp_unslash( $_POST['content_restriction_rule_content_type'] ) ) );
				$content_name = $this->get_membership_plan_rule_content_type_name( sanitize_text_field( wp_unslash( $_POST['content_restriction_rule_content_type'] ) ) );
				...
				if ( ! empty( $content_type ) && ! empty( $content_name ) ) {
					...
					$rules[] = new \WC_Memberships_Membership_Plan_Rule( array(
						...
						'object_ids'         => $this->parse_membership_plan_rule_object_ids( isset( $_POST['content_restriction_rule_object_ids'] ) ? $_POST['content_restriction_rule_object_ids'] : array() ), // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					) );
				}
			}
			...
			if ( isset( $_POST['restrict_products'], $_POST['product_restriction_rule_content_type'] ) && 'yes' === $_POST['restrict_products'] ) {
				...
				$product_type  = $this->get_membership_plan_rule_content_type_group( sanitize_text_field( wp_unslash( $_POST['product_restriction_rule_content_type'] ) ) );
				$product_group = $this->get_membership_plan_rule_content_type_name( sanitize_text_field( wp_unslash( $_POST['product_restriction_rule_content_type'] ) ) );
				...
				if ( ! empty( $product_type ) && ! empty( $product_group ) ) {
					...
					$rules[] = new \WC_Memberships_Membership_Plan_Rule( array(
						...
						'access_type'        => isset( $_POST['products_restriction_rule_access_type'] ) && in_array( $_POST['restrict_products'], array( 'view', 'purchase' ), true ) ? $_POST['products_restriction_rule_access_type'] : 'purchase', // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
						'object_ids'         => $this->parse_membership_plan_rule_object_ids( isset( $_POST['product_restriction_rule_object_ids'] ) ? $_POST['product_restriction_rule_object_ids'] : array() ), // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					) );
				}
			}
			...
			if ( isset( $_POST['offer_discounts'], $_POST['purchasing_discount_rule_content_type'] ) && 'yes' === $_POST['offer_discounts'] ) {
				...
				$product_type  = $this->get_membership_plan_rule_content_type_group( sanitize_text_field( wp_unslash( $_POST['purchasing_discount_rule_content_type'] ) ) );
				$product_group = $this->get_membership_plan_rule_content_type_name( sanitize_text_field( wp_unslash( $_POST['purchasing_discount_rule_content_type'] ) ) );
				...
				if ( ! empty( $product_type ) && ! empty( $product_group ) ) {
					...
					$rules[] = new \WC_Memberships_Membership_Plan_Rule( array(
						...
						'content_type_name'  => $product_group,
						'active'             => 'yes',
						'discount_type'      => isset( $_POST['purchasing_discount_rule_discount_type'] ) && in_array( $_POST['purchasing_discount_rule_discount_type'], array( 'percentage', 'amount' ), true ) ? $_POST['purchasing_discount_rule_discount_type'] : 'percentage', // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
						'discount_amount'    => max( 0, isset( $_POST['purchasing_discount_rule_discount_amount'] ) && is_numeric( $_POST['purchasing_discount_rule_discount_amount'] ) ? (float) $_POST['purchasing_discount_rule_discount_amount'] : 0 ), // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
						'object_ids'         => $this->parse_membership_plan_rule_object_ids( isset( $_POST['purchasing_discount_rule_object_ids'] ) ? $_POST['purchasing_discount_rule_object_ids'] : array() ), // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					) );
				}
			}
		...
		<span class="preferences" <?php if ( ! $show_preferences ) { echo 'style="display:none;"'; } ?>>
			<label>
				...
				<input
					...
					min="1"
					step="1"
					value="<?php echo esc_attr( $this->get_plugin()->get_user_memberships_instance()->get_ending_soon_days() ); ?>"
				/>
			</label>
		</span>
		<?php
		...
		<span class="preferences" <?php if ( ! $show_preferences ) { echo 'style="display:none;"'; } ?>>
			<label>
				...
				<input
					...
					min="1"
					step="1"
					value="<?php echo esc_attr( $this->get_plugin()->get_user_memberships_instance()->get_renewal_reminder_days() ); ?>"
				/>
			</label>
		</span>
		<?php
