...
class WC_Memberships_Meta_Box_Membership_Plan_Data extends \WC_Memberships_Meta_Box {
			...
			<div class="options_group">
				...
				<p class="form-field plan-access-method-field">
					...
					<span class="plan-access-method-selectors">
						...
						echo wc_help_tip( esc_html__( 'Choose how customers will gain access to this membership plan. Memberships can always be manually assigned.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				<p class="form-field js-show-if-access-method-purchase <?php if ( 'purchase' !== $current_access_type ) : ?>hide<?php endif; ?>">
					...
					<?php echo wc_help_tip( esc_html__( 'Leave empty to only allow members you manually assign.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<?php echo wp_kses_post( $this->get_trashed_products_message( $products ) ); ?>
				</p>
			...
			<div class="options_group">
				...
				<p class="form-field plan-access-length-field">
					...
					<span class="plan-access-length-selectors">
						...
						echo wc_help_tip( esc_html__( 'When does the membership expire?', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					<span class="plan-access-length-specific js-show-if-access-length-type-specific <?php if ( 'specific' !== $current_access_length ) : ?>hide<?php endif;?>">
						...
						<span>
							...
							<?php echo wc_help_tip( esc_html__( 'The membership will end after the specified length of time.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						</span>
					</span>
					...
					<span class="plan-access-length-fixed js-show-if-access-length-type-fixed <?php if ( 'fixed' !== $current_access_length ) : ?>hide<?php endif;?>">
						...
						<span>
							...
							><span class="description"><?php echo '<code>YYYY-MM-DD</code> '. wc_help_tip( esc_html__( 'The membership will start on the beginning of this day.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
						</span>
						<span>
							...
							/><span class="description"><?php echo '<code>YYYY-MM-DD</code> ' . wc_help_tip( esc_html__( 'The membership will end on the beginning of this day.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
						</span>
					</span>
		 ...
		 if ( ! empty( $trashed_products ) ) :
			...
			<span class="description" style="clear:left;display:block;">
				<?php printf(
					...
					esc_html( _n( 'The product %s is in the trash and may not be purchasable.', 'The products %s are in the trash and may not be purchasable.', count( $trashed_products ), 'woocommerce-memberships') ),
					wp_kses_post( Strings_Helper::get_human_readable_items_list( $trashed_products, 'and' ) )
				); ?>
			</span>
		    <?php
	...
	private function output_restrict_content_panel( $membership_plan, $post ) {
		...
		<div id="membership-plan-data-restrict-content" class="panel woocommerce_options_panel">
			...
			if ( ! empty( $public_posts ) ) {
				...
				printf( '<p>' . esc_html( __( 'These posts are public, and will be excluded from all restriction rules: %s', 'woocommerce-memberships' ) ) . '<p>', wp_kses_post( $this->list_post_links( $public_posts ) ) );
			}
	...
	private function output_restrict_products_panel( $membership_plan, $post ) {
		...
		<div id="membership-plan-data-restrict-products" class="panel woocommerce_options_panel">
			...
			if ( ! empty( $public_products ) ) {
				/* translators: Placeholder: %s - links to posts */
				printf( '<p>' . esc_html( __( 'These products are public, and will be excluded from all restriction rules: %s', 'woocommerce-memberships' ) ) . '</p>', wp_kses_post( $this->list_post_links( $public_products ) ) );
			}
	...
	private function output_purchasing_discounts_panel( $membership_plan, $post ) {
		...
		<div id="membership-plan-data-purchasing-discounts" class="panel woocommerce_options_panel">
			...
			if ( ! empty( $excluded_products ) ) {
				/* translators: Placeholder: %s - links to posts */
				printf( '<p>' . esc_html( __( 'These products are excluded from all member discount rules: %s', 'woocommerce-memberships' ) ) . '</p>', wp_kses_post( $this->list_post_links( $excluded_products ) ) );
			}
	...
	private function output_members_area_panel( $membership_plan, $post ) {
		...
		<div id="membership-plan-members-area" class="panel woocommerce_options_panel">
			...
			<p class="form-field">
				...
				<?php echo wc_help_tip( esc_html__( 'Leave empty to hide members area for this membership.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		...
		<div id="membership-plan-email-content" class="panel woocommerce_options_panel">
			...
			<?php foreach ( $emails as $id => $email ) :
				...
				<div class="options_group" style="padding: 10px;">
					<h4><?php echo esc_html( $email->get_title() ); ?> <small>(<a href="<?php echo esc_url( $email_setting_link ); ?>"><?php echo esc_html( strtolower( __( 'Configure Email', 'woocommerce-memberships' ) ) ); ?></a>)</small></h4>
			...
			<?php if ( 0 === $enabled ) : ?>
				...
				<p><em><?php /* translators: Placeholder: %1$s - opening <a> link tag, %2$s - closing </a> link tag */
						printf( esc_html__( 'It looks like you haven\'t enabled any of the Memberships emails. To configure email content for this plan, you need to enable at least one membership email from the %1$sWooCommerce Emails settings%2$s', 'woocommerce-memberships' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=email' ) ) . '">', '</a>' ); ?></em></p>
	...
	public function update_data( $post_id, \WP_Post $post ) {
		...
		if (    $membership_plan
		     ...
		     && in_array( sanitize_text_field( $_POST['_access_method'] ), $access_methods, true ) ) {

			// save access method type
			$membership_plan->set_access_method( sanitize_text_field( wp_unslash( $_POST['_access_method'] ) ) );
			...
			if ( ! empty( $_POST['_access_length'] ) ) {
				...
				if (    'specific' === $_POST['_access_length']
					...
					$access_length = sprintf( '%d %s',
						...
						sanitize_text_field( wp_unslash( $_POST['_access_length_period'] ) )
					);
					...
					$membership_plan->set_access_length( $access_length );
				           ...
				           && ( $access_start_date = wc_memberships_parse_date( sanitize_text_field( wp_unslash( $_POST['_access_start_date'] ) ), 'mysql' ) )
				           && ( $access_end_date   = wc_memberships_parse_date( sanitize_text_field( wp_unslash( $_POST['_access_end_date'] ) ), 'mysql' ) ) ) {
			...
			if ( $membership_plan->is_access_method( 'purchase' ) ) {
				...
				if ( ! empty( $_POST['_product_ids'] ) ) {
					...
					$membership_plan->set_product_ids( array_map( 'sanitize_text_field', (array) $_POST['_product_ids'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash
		...
		foreach ( wc_memberships()->get_emails_instance()->get_email_class_names() as $email_id ) {
			if ( isset( $_POST[ $email_id ] ) ) {
				$membership_plan->set_email_content( $email_id, wp_kses_post( wp_unslash( $_POST[ $email_id ] ) ) );
			}
		}
