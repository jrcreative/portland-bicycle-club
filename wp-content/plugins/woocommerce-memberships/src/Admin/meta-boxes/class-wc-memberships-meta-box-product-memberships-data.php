...
class WC_Memberships_Meta_Box_Product_Memberships_Data extends \WC_Memberships_Meta_Box {
			...
			<div class="js-restrictions <?php if ( 'yes' === wc_memberships_get_content_meta( $post->ID, '_wc_memberships_force_public', true ) ) : ?>hide<?php endif; ?>">
				...
				<div class="options_group">
					...
					<div
						...
						printf( '<p>' . esc_html__( '%1$s automatically inserts the product(s) needed to gain access. %2$s inserts the URL to my account page. HTML is allowed.', 'woocommerce-memberships' ) . '</p>',
	...
	private function output_grant_access_panel( $product, $post ) {
		...
		<div id="memberships-data-grant-access" class="panel woocommerce_options_panel">
			...
			<p class="variable-notice <?php if ( ! $product->is_type( 'variable' ) ) : ?>hide<?php endif; ?>">
				...
				printf( esc_html__( 'These settings affect all variations. For variation-level control use the %1$smembership plan screen%2$s.', 'woocommerce-memberships' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=wc_membership_plan' ) ) . '">', '</a>' ); ?>
			</p>
			...
			<div class="options_group">
				...
				<?php if ( empty( $membership_plans ) ) : ?>
					...
					<p>
						...
						printf( esc_html__( 'To grant membership access, please %1$sAdd a Membership Plan%2$s', 'woocommerce-memberships' ), '<a target="_blank" href="' . esc_url( admin_url( 'post-new.php?post_type=wc_membership_plan' ) ) . '">', '</a>' ); ?>
					</p>
					...
					<p class="form-field">
						...
						<select
							...
							<?php foreach ( $grant_access_to_plans as $plan ) : ?>
								<option value="<?php echo esc_attr( $plan->get_id() ); ?>" selected><?php echo esc_html( $plan->get_formatted_name() ); ?></option>
							<?php endforeach; ?>
						</select>
						<?php echo wc_help_tip( esc_html__( 'Select which membership plans does purchasing this product grant access to.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</p>
	...
	public function update_data( $post_id, \WP_Post $post ) {
		...
		$posted_plan_ids = isset( $_POST['_wc_memberships_membership_plan_ids'] ) ? $_POST['_wc_memberships_membership_plan_ids'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
