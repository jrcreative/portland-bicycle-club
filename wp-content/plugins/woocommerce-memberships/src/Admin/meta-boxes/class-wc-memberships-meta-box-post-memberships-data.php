		...
		<div class="js-restrictions <?php if ( 'yes' === wc_memberships_get_content_meta( $post, '_wc_memberships_force_public', true ) ) : ?>hide<?php endif; ?>">
			...
			<div
				...
				<p>
					...
					printf( esc_html__( '%1$s automatically inserts the product(s) needed to gain access. %2$s inserts the URL to my account page. HTML is allowed.', 'woocommerce-memberships' ), '<strong><code>{products}</code></strong>', '<strong><code>{login_url}</code></strong>' ); ?>
				</p>
				<?php
