...
class WC_Memberships_Admin_Membership_Plans {
	...
	public function custom_column_content( $column, $post_id ) {
		...
		if ( $membership_plan ) {
			...
			switch ( $column ) {
				...
				case 'slug':
					echo esc_html( $membership_plan->get_slug() );
				break;
				...
				case 'length':
					...
					if ( 'purchase' === $membership_plan->get_access_method() && 0 === count( $has_products ) ) {
						...
						echo esc_html( $membership_plan->get_human_access_length() );
					}
				...
				case 'rules':
					...
					if ( defined( 'WP_DEBUG' ) && true === WP_DEBUG ) {
						...
						foreach ( $rules as $label => $active ) {
							...
							printf( "<span class='".esc_attr( $class )."'>%s ".esc_html( $label )."</span><br>", ! $active ? '&#x2717;' : '&#x2713;' );
						}
					}
				...
				case 'members':
					...
					echo esc_html( $membership_plan->get_memberships_count() );
					echo '</a>';
		...
		if ( ! empty( $product_ids ) ) {
			...
			foreach ( $product_ids as $product_id ) {
				...
				if ( $product = wc_get_product( $product_id ) )  {
					...
					printf(
						'<li>%1$s%2$s</li>',
						$this->get_edit_product_link( $product ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						$is_subscription ? ' <small>(' . strtolower( esc_html__( 'Subscription', 'woocommerce-memberships' ) ) . ')</small> ' : '' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					);
				}
			}
					...
					switch ( $plan->get_access_method() ) {
						...
						case 'signup' :
							...
							<span class="grant-access-signup"><?php
								...
								printf( esc_html__( 'Existing users: %s', 'woocommerce-memberships' ), $grant_access_button ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
							<?php
						...
						default :
							...
							<span class="grant-access-purchase"><?php
								/* translators: Placeholder: %s - HTML button */
								printf( esc_html__( 'Existing purchases: %s', 'woocommerce-memberships' ), $grant_access_button  ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
							<?php
	...
	public function duplicate_membership_plan() {
		...
		wp_die( sprintf( esc_html__( 'Membership plan creation failed, could not find original plan to copy: %d', 'woocommerce-memberships' ), (int) $id ) );
	}
