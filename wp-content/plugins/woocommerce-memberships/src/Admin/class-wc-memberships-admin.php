...
class WC_Memberships_Admin {
	...
	public function remove_submenu_link() {
		...
		if (    ( 'edit.php' === $pagenow && 'wc_membership_plan' === $typenow )
		     || ( 'post.php' === $pagenow && isset( $_GET['post'] ) && 'wc_membership_plan' === get_post_type( absint( $_GET['post'] ) ) ) ) {
					...
					if ( ! empty( $has_atts ) ) {
						...
						<div class="notice notice-warning">
							<p><?php
								...
								printf( esc_html__( 'It looks like you might be using the %1$s shortcode with deprecated attributes, such as %2$s. These attributes have been deprecated since WooCommerce 2.6 and may no longer have any effect on the shortcode output. Furthermore, they might cause a server error when visiting the Members Area while WooCommerce Memberships is active.', 'woocommerce-memberships' ), '<code>woocommerce_my_account</code>', '<code>order_count</code>' );
								?></p>
						</div>
						<?php
					}
