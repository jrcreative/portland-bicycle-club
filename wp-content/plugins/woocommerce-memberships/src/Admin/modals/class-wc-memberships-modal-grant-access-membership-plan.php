...
class WC_Memberships_Modal_Grant_Access_Membership_Plan extends \WC_Memberships_Batch_Job_Modal {
	...
	protected function get_job_progress() {
		...
		echo parent::get_job_progress(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo $this->get_progress_bar(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	protected function get_progress_bar() {
		...
		ob_start();
			...
			echo parent::get_progress_bar(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

		?>
		<div>
			<# if ( data.job.total == 1 ) { #>
				<p><small><?php /* translators: Placeholders: %s - current progress (number), either 0 or 1 */
				printf( esc_html__( 'Processed %s out of 1 user.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?></small></p>
			<# } else { #>
				<p><small><?php /* translators: Placeholders: %1$s - current progress (number), %2$s - total amount of users to process (number) */
				printf( esc_html__( 'Processed %1$s out of %2$s users.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?></small></p>
			<# } #>
		</div>
		<?php
	...
	protected function get_job_completed() {
		...
		<?php echo $this->get_progress_bar(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
