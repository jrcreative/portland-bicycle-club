...
class WC_Memberships_User_Membership implements \SkyVerge\WooCommerce\PluginFramework\v6_2_1\Abilities\Contracts\JsonSerializable {
	...
	/**
	 * Returns a serializable representation of this user membership.
	 *
	 * @since 1.28.0
	 *
	...
	{
		return UserMembershipSerializer::convert($this);
	}
	...
	{
		return UserMembershipSerializer::getJsonSchema();
	}
	...
	public function set_start_date( $date = '' ) {
		...
		if ( $plan && $plan->is_access_length_type( 'fixed' )  ) {
			...
			try {
				...
				trigger_error( esc_html( sprintf( 'Unable to set start date for User Membership #%d: %s', $this->get_id(), $e->getMessage() ) ), E_USER_WARNING );
			}
		}
		...
		if ( ! empty( $end_timestamp ) ) {
			...
			if ( $plan && $plan->is_access_length_type( 'fixed' )  ) {
				...
				try {
					...
					trigger_error( esc_html( sprintf( 'Unable to end start date for User Membership #%d: %s', $this->get_id(), $e->getMessage() ) ), E_USER_WARNING );
				}
			}
	...
	public function transfer_ownership( $to_user ) {
		...
		if ( ! empty( $error ) ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( esc_html( current( $error ) ), esc_html( key( $error ) ) );
		}
		...
		if ( (int) $this->id !== (int) $updated ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html( current( $default_error ) ), esc_html( key( $default_error ) ) );
		}
