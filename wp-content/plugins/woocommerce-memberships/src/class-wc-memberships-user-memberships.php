...
class WC_Memberships_User_Memberships {
	...
	public function create_user_membership( $args = array(), $action = 'create' ) {
		...
		if ( ! wc_memberships_get_membership_plan( $args['plan_id'] ) ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( esc_html( sprintf( __( 'Cannot create User Membership: Membership Plan with ID %d does not exist', 'woocommerce-memberships' ), (int) $args['plan_id'] ) ) );
		}
		...
		if ( 0 === $user_membership_id || is_wp_error( $user_membership_id ) ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( esc_html( sprintf( __( 'Cannot create User Membership: %s.', 'woocommerce-memberships' ), implode( ', ', $user_membership_id->get_error_messages() ) ) ) );
		}
		...
		if ( ! $user_membership instanceof WC_Memberships_User_Membership) {
			/* translators: Placeholder: %d - membership plan ID */
			throw new Framework\SV_WC_Plugin_Exception( esc_html( sprintf( __( 'Cannot create User Membership #%d.', 'woocommerce-memberships' ), $user_membership_id ) ) );
		}
	...
	public function delete_user_memberships( $user_id ) {
		...
		foreach ( $user_memberships as $membership ) {
			try {
				$this->deleteUserMembership($membership);
			} catch(Exception $e) {
				// do nothing, for backwards compatibility (this previously didn't throw an exception)
			}
		}
	}
	...
	/**
	 * Deletes a single user membership by ID.
	 *
	 * @since 1.28.0
	 *
	 * @param int|WC_Memberships_User_Membership $membership ID or object of the user membership to delete
	 * @return WC_Memberships_User_Membership the deleted membership
	 * @throws UserMembershipNotFoundException
	 * @throws UserMembershipDeleteFailedException
	 */
	public function deleteUserMembership($membership): WC_Memberships_User_Membership
	{
		// in case we were passed an ID
		if (! $membership instanceof WC_Memberships_User_Membership && is_numeric($membership)) {
			$membership = $this->get_user_membership($membership);
		}

		if (! $membership instanceof WC_Memberships_User_Membership) {
			throw new UserMembershipNotFoundException();
		}

		$result = wp_delete_post($membership->get_id(), true);

		if (! $result) {
			throw new UserMembershipDeleteFailedException();
		}

		$this->prune_object_caches($membership);
	...
	public function delete_related_data( $post_id ) {
		...
		foreach ( $other_user_memberships as $other_user_membership ) {
			...
			if ( $other_user_membership->get_id() !== $user_membership->get_id() && $other_user_membership->is_active() ) {
