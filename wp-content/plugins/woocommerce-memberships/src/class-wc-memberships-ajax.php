...
class WC_Memberships_AJAX {
		...
		if ( isset( $_POST['plan'] ) ) {
			...
			if ( $plan  = wc_memberships_get_membership_plan( $plan_id ) ) {
				...
				if ( 'start' === $which_date ) {
					...
					$start_date     = ! empty( $_POST['start_date'] ) ? strtotime( sanitize_text_field( wp_unslash( $_POST['start_date'] ) ) ) : current_time( 'timestamp', true );
	...
	public function add_user_membership_note() {
		...
		$post_id   = (int) wp_unslash( $_POST['post_id'] ?? 0 ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$note_text = wp_kses_post( wp_unslash( $_POST['note'] ?? '' ) );
	...
	public function create_user_for_membership() {
		...
		$password   = isset( $_POST['password']   ) ? $_POST['password'] : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function toggle_member_profile_field_editable_by() {
		...
		$profile_field_definition = Profile_Fields::get_profile_field_definition( isset( $_POST['profile_field'] ) ? sanitize_text_field( wp_unslash( $_POST['profile_field'] ) ) : '' );
	...
	public function upload_member_profile_field_file() {
		...
		if ( empty( $_FILES ) || ( isset( $_FILES['file']['error'] ) && ! empty( $_FILES['file']['error'] ) ) ) {
			return false;
		}
		...
		$file_info = pathinfo( sanitize_text_field( $_FILES['file']['name'] ?? '' ) );
	...
	public function get_batch_job() {
		...
		if ( isset( $_POST['job_name'], $_POST['job_id'] ) && is_string( $_POST['job_name'] ) && ( is_string( $_POST['job_id'] ) || is_numeric( $_POST['job_id'] ) ) ) {
			...
			$job_name = trim( sanitize_text_field( wp_unslash( $_POST['job_name'] ) ) );
			$job_id   = is_numeric( $_POST['job_id'] ) ? (int) $_POST['job_id'] : trim( sanitize_text_field( wp_unslash( $_POST['job_id'] ) ) );
	...
	public function remove_batch_job() {
		...
		$error    = esc_html__( 'Must specify a valid job process name and job ID.', 'woocommerce-memberships' );
		$job_name = ! empty( $_POST['job_name'] ) && is_string( $_POST['job_name'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['job_name'] ) ) ) : null;
		$job_id   = isset( $_POST['job_id'] ) &&
					( null === $_POST['job_id'] || is_string( $_POST['job_id'] ) || is_numeric( $_POST['job_id'] ) )
	...
	public function grant_retroactive_access() {
		...
		if ( isset( $_POST['plan_id'] ) && is_numeric( $_POST['plan_id'] ) ) {
			...
			$error = sprintf( esc_html__( 'Could not get a valid published plan with ID %s.', 'woocommerce-memberships' ), $plan_id );
	...
	public function export_user_memberships() {
		...
		$export_args    = isset( $_POST['export_params'] ) && is_array( $_POST['export_params'] ) ? $_POST['export_params'] : null; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
		...
		if ( ! empty( $_FILES ) && isset( $_FILES['file']['name'], $_FILES['file']['size'], $_FILES['file']['tmp_name'] ) ) {
			...
			try {
				...
				wp_send_json_success( (array) $import_handler->create_job( array(
					...
					'timezone'                   => ! empty( $_POST['timezone'] )                 ? sanitize_text_field( wp_unslash( $_POST['timezone'] ) )                             : wc_timezone_string(),
					'default_start_date'         => ! empty( $_POST['default_start_date'] )       ? sanitize_text_field( wp_unslash( $_POST['default_start_date'] ) )                   : date( 'Y-m-d', current_time( 'timestamp' ) ),
					'fields_delimiter'           => ! empty( $_POST['fields_delimiter'] )         ? sanitize_text_field( wp_unslash( $_POST['fields_delimiter'] ) )                     : 'comma',
					'date_format'                => ! empty( $_POST['date_format'] )              ? sanitize_text_field( wp_unslash( $_POST['date_format'] ) )                          : '',
					'custom_date_format'         => ! empty( $_POST['custom_date_format'] )       ? sanitize_text_field( wp_unslash( $_POST['custom_date_format'] ) )                   : '',
				) ) );
