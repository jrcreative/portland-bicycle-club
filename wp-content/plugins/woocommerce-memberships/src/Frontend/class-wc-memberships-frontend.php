...
class WC_Memberships_Frontend {
	...
	public function output_admin_message_html() {
		...
		echo wp_kses_post( \WC_Memberships_User_Messages::get_admin_message_html() );
	}
	...
	public function cancel_membership() {
		...
		if ( ! $user_membership ) {
			...
			if (     current_user_can( 'wc_memberships_cancel_membership', $user_membership_id )
			      ...
			      && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'wc_memberships-cancel_membership_' . $user_membership_id ) ) {
	...
	private function log_member_in( $user_membership ) {
		...
		if ( ! ( $current_user_id > 0 && $log_in_user_id === $current_user_id ) ) {
			...
			if ( ! $allow_login ) {
				throw new Framework\SV_WC_Plugin_Exception( esc_html( $error_message ) );
			}

			// finally log in the user
			wp_set_current_user( $log_in_user_id ); // phpcs:ignore Generic.PHP.ForbiddenFunctions.Discouraged
			// nosemgrep: audit.php.wp.security.auth-bypass.wp-set-auth-cookie
			wp_set_auth_cookie( $log_in_user_id ); // phpcs:ignore Generic.PHP.ForbiddenFunctions.Discouraged
		}
	...
	public function renew_membership() {
		...
		$error_display      = isset( $_REQUEST['display'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['display'] ) ) : 'all';
		...
		if ( isset( $_REQUEST['error'] ) ) {
			...
			if ( '' !== $_REQUEST['success'] ) {
				wc_add_notice( sanitize_text_field( wp_unslash( $_REQUEST['success'] ) ), 'success' );
			}

		} else {

			$user_membership = wc_memberships_get_user_membership( $user_membership_id );
			$user_token      = isset( $_REQUEST['user_token'] ) ? wc_clean( wp_unslash( $_REQUEST['user_token'] ) ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	protected function process_membership_renewal( $user_membership, $token ) {
		...
		if ( is_string( $error_message ) && '' !== trim( $error_message ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html( $error_message ), is_user_logged_in() ? 1 : 0 );
		}
