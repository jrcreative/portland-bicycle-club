...
class Members_Area {
	...
	private function get_members_area_query_vars() {
		...
		if ( ! $this->using_permalinks ) {
			...
			if ( isset( $_GET['members_area_section'] ) ) {
				$query_vars[] = sanitize_text_field( wp_unslash( $_GET['members_area_section'] ) );
			}
			if ( isset( $_GET['members_area_section_page'] ) && is_numeric( $_GET['members_area_section_page'] ) ) {
				$query_vars[] = (int) sanitize_text_field( wp_unslash( $_GET['members_area_section_page'] ) );
			}
		} else {
	...
	public function get_members_area_sorting_args() {
		...
		if ( isset( $_GET['sort_by'] ) && in_array( sanitize_text_field( wp_unslash( $_GET['sort_by'] ) ), array( 'title', 'type' ), true ) ) {

			$args['orderby'] = sanitize_text_field( wp_unslash( $_GET['sort_by'] ) );

			if ( isset( $_GET['sort_order'] ) && in_array( strtoupper( sanitize_text_field( wp_unslash( $_GET['sort_order'] ) ) ), array( 'ASC', 'DESC' ), true ) ) {
				$args['order'] = strtoupper( sanitize_text_field( wp_unslash( $_GET['sort_order'] ) ) );
			} else {
				$args['order'] = 'ASC';
			}
		}
