...
class User_Meta implements \WC_Object_Data_Store_Interface {
	...
	public function create( &$profile_field ) {
		...
		if ( ! $profile_field instanceof Profile_Field ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'Invalid profile field.', 'woocommerce-memberships' ) );
		}
	...
	public function read( &$profile_field ) {
		...
		if ( empty( $user_id ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'Invalid profile field user ID.', 'woocommerce-memberships' ) );
		}

		if ( empty( $meta_key ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'Invalid profile field slug.', 'woocommerce-memberships' ) );
		}

		if ( ! metadata_exists( 'user', $user_id, $meta_key ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'Profile field not found.', 'woocommerce-memberships' ) );
		}
