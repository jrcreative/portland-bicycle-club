...
class Option implements \WC_Object_Data_Store_Interface {
	...
	public function create( &$profile_field_definition ) {
		...
		if ( ! $profile_field_definition instanceof Profile_Field_Definition ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'Invalid profile field definition.', 'woocommerce-memberships' ) );
		}
	...
	public function read( &$profile_field_definition ) {
		...
		if ( ! isset( $profile_field_definitions[ $profile_field_slug ] ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'Profile field definition not found.', 'woocommerce-memberships' ) );
		}
	...
	public function update( &$profile_field_definition ) {
		...
		if ( empty( $profile_field_definition->get_id() ) ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'Profile field definition ID invalid or not found while updating.', 'woocommerce-memberships' ) );
		}
