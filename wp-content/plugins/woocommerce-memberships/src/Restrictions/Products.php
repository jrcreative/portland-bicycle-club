...
class Products {
			...
			if (    ! current_user_can( 'wc_memberships_view_restricted_post_content', $shop_page_id )
				...
				switch ( $restrictions->get_restriction_mode() ) {
					...
					case 'redirect' :
						...
						wp_safe_redirect( $restrictions->get_restricted_content_redirect_url( $shop_page_id, 'post_type', 'page' ) );
						exit;
	...
	public function template_loop_product_thumbnail_placeholder() {
		...
		if ( wc_placeholder_img_src() ) {
			...
			echo wc_placeholder_img( 'shop_catalog' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	...
	public function display_product_purchasing_restricted_message() {
		...
		if ( $product instanceof WC_Product ) {
			...
			if ( ! current_user_can( 'wc_memberships_purchase_restricted_product', $product_id ) ) {
				...
				echo \WC_Memberships_User_Messages::get_message_html( 'product_purchasing_restricted', $args ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				echo \WC_Memberships_User_Messages::get_message_html( 'product_access_delayed', $args ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	public function display_product_purchasing_discount_message() {
		...
		if ( $product instanceof WC_Product ) {
			...
			if (      wc_memberships_product_has_member_discount( $product )
				...
				echo \WC_Memberships_User_Messages::get_message_html( 'product_discount', $args ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
