...
class Posts {
		...
		if ( $restricted ) {
			...
			if ( 'post_type' === $object_type ) {
				...
				if (    isset( $_GET['wc-ajax'] )
				     ...
				     && has_action( 'wp_ajax_nopriv_woocommerce_' . sanitize_text_field( wp_unslash( $_GET['wc-ajax'] ?? '' ) ) ) ) {
			...
			wp_safe_redirect( wc_memberships()->get_restrictions_instance()->get_restricted_content_redirect_url( $object_id, $object_type, $object_type_name ) );
			exit;
		}
	}
		...
		if ( $wp_query instanceof \WP_Query && $wp_query->is_archive() && ! current_user_can( 'wc_memberships_access_all_restricted_content' ) ) {
			...
			if ( $term instanceof \WP_Term ) {
				...
				if ( '' !== $message_code ) {
				    ...
				    echo \WC_Memberships_User_Messages::get_message_html( $message_code, $args ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
		}
	}
	...
	public function redirect_to_member_content_upon_login( $redirect_to ) {
		...
		if ( isset( $_GET['wcm_redirect_to'], $_GET['wcm_redirect_id'] ) ) {
			...
			$redirect_to = sanitize_text_field( wp_unslash( $_GET['wcm_redirect_to'] ?? '' ) );
			$redirect_id = (int) ( $_GET['wcm_redirect_id'] ?? 0 ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

			if ( in_array( $redirect_to, get_post_types(), true ) ) {
				$restricted_content_link = get_permalink( $redirect_id );
			} elseif ( taxonomy_exists( $redirect_to ) ) {
				$restricted_content_link = get_term_link( $redirect_id, $redirect_to );
			}
		}
	...
	private function exclude_restricted_posts_types( array $restricted_post_types ) {
		...
		if ( empty( $granted_posts ) && empty( $granted_terms ) && empty( $granted_taxonomies ) ) {
			...
			$clause     = $wpdb->prepare( " AND $wpdb->posts.post_type NOT IN ($post_types) ", $restricted_post_types ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
		...
		// there are extra rules that grant the user access to some taxonomies, terms or posts in one or more restricted post types
			...
			$subquery  = $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type IN ($post_types)", $restricted_post_types ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
	...
	private function exclude_restricted_taxonomies( array $restricted_taxonomies ) {
		...
		if ( ! empty( $taxonomy_post_types ) ) {
			...
			foreach ( $taxonomy_post_types as $tax => $post_types ) {
				...
				$case .= $wpdb->prepare( " WHEN $wpdb->term_taxonomy.taxonomy = %s THEN $wpdb->posts.post_type IN ( $post_types_placeholder )", $args ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
			}
	...
	private function get_term_access_where_clause( $term_ids, $query_type = 'posts' ) {
		...
		if ( ! empty( $term_ids ) ) {
			...
			if ( 'posts' === $query_type ) {
				...
				$clause = $wpdb->prepare( " AND sub_t.term_id NOT IN ($placeholder) ", $term_ids ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
			}
		}
	...
	private function get_post_access_where_clause( $post_ids ) {
		...
		if ( ! empty( $post_ids ) ) {
			...
			$clause      = $wpdb->prepare( " AND ID NOT IN ($placeholder)", $post_ids ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
		} else {
			$clause      = '';
		}
	...
	private function restrict_post_comments( \WP_Post $post ) {
		...
		// Add filters only for this specific post and request
		add_filter( 'comments_open', function( $open, $post_id ) use ( $post ) {
			return ( $post_id == $post->ID ) ? false : $open;
		}, 10, 2 );
  
		add_filter( 'get_comments_number', function( $count, $post_id ) use ( $post ) {
			return ( $post_id == $post->ID ) ? 0 : $count;
		}, 10, 2 );
	}
