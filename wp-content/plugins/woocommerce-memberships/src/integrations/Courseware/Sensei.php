...
class Sensei extends Courseware {
	...
	protected function has_user_completed_course_prerequisites( int $user_id, int $course_id ) : bool {
		...
		if ( ! is_callable( 'Sensei_Utils::user_completed_course' ) ) {
			return false;
		}
		...
		if ( ! empty( $course_prerequisite_id ) ) {
			...
			$prerequisite_complete = \Sensei_Utils::user_completed_course( $course_prerequisite_id, $user_id );

		}

		/**
		 * Filter course prerequisite complete. Applied here for consistency with Sensei's own handling.
		 ...
		 */
		return (bool) apply_filters( 'sensei_course_is_prerequisite_complete', $prerequisite_complete, $course_id );
