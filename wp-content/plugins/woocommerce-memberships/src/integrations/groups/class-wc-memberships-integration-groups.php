...
class WC_Memberships_Integration_Groups {
				...
				<?php foreach ( $this->get_groups() as $group ) : ?>
					...
					<fieldset>
						<h4><?php printf( /* translators: %s - Group name, %d - Group ID */
								esc_html__( 'Group: %s (ID #%d) ', 'woocommerce-memberships' ), esc_html( $group->name ), esc_html( $group->group_id ) ); ?></h4>
	...
	public function import_groups() {
		...
		foreach ( $_POST['import_groups'] as $group_id => $map ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ) ?>">
				<a id="<?php echo esc_attr( $value['id'] ); ?>"
				   ...
				   class="<?php echo esc_attr( $value['class'] ); ?>"
					<?php echo implode( ' ', $custom_attributes ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php echo esc_html( $value['default'] ); ?></a>
	...
	private function get_groups() {
		...
		return (array) $wpdb->get_results( "SELECT group_id, name FROM $group_table ORDER BY group_id" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	}
	...
	private function get_group_members( $group_id ) {
		...
		return (array) $wpdb->get_col( $wpdb->prepare( "SELECT user_id FROM $table WHERE group_id=%d", $group_id ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	}
