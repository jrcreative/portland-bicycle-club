	...
	private function get_temporary_plan_member( \WC_Memberships_Membership_Plan $membership_plan ) {
		...
		if ( null === $user ) {
			...
			if ( $user_id instanceof \WP_Error ) {
				throw new Framework\SV_WC_Plugin_Exception( esc_html( $user_id->get_error_message() ) );
			}
		...
		if ( ! $user instanceof \WP_User ) {
			throw new Framework\SV_WC_Plugin_Exception( esc_html__( 'A temporary plan member could not be determined.', 'woocommerce-memberships' ) );
		}
