	...
	public function membership_plan_data_validate_subscription_args( $data ) {
		...
		if ( isset( $data['subscription_length'] ) ) {
			...
			if ( 'unlimited' === $length ) {
				...
				throw new \WC_CLI_Exception( 'woocommerce_memberships_cli_invalid_plan_subscription_length', sprintf( 'Membership Plan length "%s" is not valid. Must be "unlimited" or in "<amount> <period>" format.', esc_html( $data['length'] ) ) );
		...
		if ( isset( $data['subscription_start_date'] ) || isset( $data['subscription_end_date'] ) ) {
			...
			if ( ! $start_date ) {
				throw new \WC_CLI_Exception( 'woocommerce_memberships_cli_invalid_subscription_start_date', sprintf( 'Membership Plan start date "%s" is not valid. Must be a non-empty YYYY-MM-DD value. Can be omitted and current date will be used.', esc_html( $data['start_date'] ) ) );
			} elseif( ! $end_date ) {
				throw new \WC_CLI_Exception( 'woocommerce_memberships_cli_invalid_subscription_end_date', sprintf( 'Membership Plan end date "%s" is not valid. Must be a non-empty YYYY-MM-DD value.', esc_html( $data['end_date'] ) ) );
	...
	public function user_membership_data_validate_subscription_args( $data ) {
		...
		if ( isset( $data['subscription'] ) ) {
			...
			if ( ! $subscription instanceof \WC_Subscription ) {
				throw new \WC_CLI_Exception( 'woocommerce_memberships_subscription_not_found', sprintf( 'Subscription %s not found.', esc_html( $data['subscription'] ) ) );
			}
