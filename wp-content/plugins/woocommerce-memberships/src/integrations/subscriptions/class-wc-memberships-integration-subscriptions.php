		...
		if ( 'end' === $date_type && ( $user_memberships = $this->get_memberships_from_subscription( $subscription->get_id() ) ) ) {
			...
			foreach ( $user_memberships as $user_membership ) {
				...
				if ( $subscription_plan_id && $this->membership_plans->grant_access_while_subscription_active( $subscription_plan_id ) ) {
					...
					/*
					 * This accounts for edge cases where the subscription end date changed, but the status did not, meaning the
					 * `woocommerce_subscription_status_updated` hook was not triggered. It ensures memberships don't get stuck in an unexpected status.
					 */
					$this->update_related_membership_status( $subscription, $user_membership, $subscription->get_status() );
				}
			}
		}
	}
