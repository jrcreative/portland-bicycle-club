	...
	public function output_subscription_options() {
		...
		<p class="form-field plan-subscription-access-length-field js-show-if-has-subscription <?php if ( ! $has_subscription ) : ?>hide<?php endif; ?>">
			...
			<span class="plan-subscription-access-length-selectors">
				...
				echo wc_help_tip( esc_html__( 'When does the membership tied to a subscription expire?', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			...
			<span class="subscription-access-notice description js-show-if-subscription-access-length-unlimited" <?php if ( 'unlimited' !== $current_subscription_access_length_type ) : ?>style="display: none;"<?php endif; ?>>
				<?php esc_html_e( 'The membership will be active indefinitely, even after the subscription billing cycle is complete, as long as it has been fully paid.', 'woocommerce-memberships' ); ?>
					<?php echo wc_help_tip( esc_html__( 'When unlimited access is granted via the purchase of a subscription, the membership will be active for the period of the subscription length, and will stay active beyond that as long as the customer successfully completed the subscription billing cycle.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</span>
			...
			<span class="subscription-access-notice description js-show-if-subscription-access-length-subscription" <?php if ( 'subscription' !== $current_subscription_access_length_type ) : ?>style="display: none;"<?php endif; ?>>
				...
				<?php echo wc_help_tip( esc_html__( 'When access is granted via the purchase of a subscription, the membership length becomes tied to the length of the subscription.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</span>
			...
			<span class="plan-access-length-specific plan-subscription-access-length-specific js-show-if-subscription-access-length-specific <?php if ( 'specific' !== $current_subscription_access_length_type ) : ?>hide<?php endif;?>">
				...
				<span class="subscription-access-notice description">
					...
					<?php echo wc_help_tip( esc_html__( 'When membership access is granted via the purchase of a subscription, then membership length will last for the specified period, regardless of the subscription length, as long as the customer pays for the subscription costs.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</span>
			...
			<span class="plan-access-length-fixed plan-subscription-access-length-fixed js-show-if-access-length-fixed <?php if ( 'fixed' !== $current_subscription_access_length_type ) : ?>hide<?php endif;?>">
				...
				<span class="subscription-access-notice description">
					...
					<?php echo wc_help_tip( esc_html__( 'When membership access is granted via the purchase of a subscription, the membership will last until the specified date, regardless of the subscription sign up date and the subscription length, as long as the customer pays for the subscription costs.', 'woocommerce-memberships' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</span>
	...
	public function output_exclude_trial_option( $rule, $index ) {
		...
		<span class="rule-control-group rule-control-group-access-schedule-trial <?php if ( ! $has_subscription ) : ?>hide<?php endif; ?> js-show-if-has-subscription">
			...
			<label class="label-checkbox">
				<input type="checkbox"
					   name="_<?php echo esc_attr( $type ); ?>_rules[<?php echo esc_attr( $index ); ?>][access_schedule_exclude_trial]"
					   id="_<?php echo esc_attr( $type ); ?>_rules_<?php echo esc_attr( $index ); ?>_access_schedule_exclude_trial"
	...
	public function membership_plan_screen_columns( $column, $post_id ) {
		...
		if (    'length' === $column
			...
			if ( ( $human_length !== $sub_human_length ) || $subscription_tied ) {
				...
				echo $membership_plan->get_products( true ) ? '<br><small>' . sprintf( esc_html__( 'Subscription based: %s', 'woocommerce-memberships' ), esc_html( $sub_human_length ) ) . '</small>' : esc_html( $sub_human_length );
			}
		}
	}
	...
	public function user_membership_screen_columns( $column, $user_membership_id ) {
		...
		if (    'member_since' === $column
			...
			if ( $subscription = $user_membership->get_subscription() ) {
				...
				printf( '<span class="member-since-subscription"><small>' . esc_html__( 'Subscription: %s', 'woocommerce-memberships' ) . '</small></span>', '<a href="' . esc_url( $subscription->get_edit_order_url() ) . '">#' . esc_html( $subscription->get_id() ) . '</a>' );
			}
		}
	}
			...
			if (    isset( $posted_data['_access_method'], $posted_data['_subscription_access_length'] )
				...
				if (    'specific' === $posted_data['_subscription_access_length']
					...
					$access_length = sprintf( '%d %s',
						max( 1, (int) ( $_POST['_subscription_access_length_amount'] ?? 0 ) ), // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
						sanitize_text_field( wp_unslash( $_POST['_subscription_access_length_period'] ?? '' ) )
					);
				...
				} elseif (    'fixed' === sanitize_text_field( wp_unslash( $_POST['_subscription_access_length'] ?? '' ) )
				           && isset( $_POST['_subscription_access_start_date'], $_POST['_subscription_access_end_date'] )
				           && ( $subscription_access_start_date = wc_memberships_parse_date( sanitize_text_field( wp_unslash( $_POST['_subscription_access_start_date'] ?? '' ) ), 'mysql' ) )
				           && ( $subscription_access_end_date   = wc_memberships_parse_date( sanitize_text_field( wp_unslash( $_POST['_subscription_access_end_date'] ?? '' ) ), 'mysql' ) ) ) {
