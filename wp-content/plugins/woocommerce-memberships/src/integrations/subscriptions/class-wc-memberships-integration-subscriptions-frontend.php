	...
	public function output_subscription_columns( $user_membership ) {
		...
		if ( $user_membership instanceof \WC_Memberships_User_Membership ) {
			...
			if ( $user_membership->has_subscription() ) {
				$next_bill_on = $user_membership->get_next_bill_on_local_date( 'timestamp' );
			}
		}

		echo null === $next_bill_on ? esc_html__( 'N/A', 'woocommerce-memberships' ) : esc_html( date_i18n( wc_date_format(), $next_bill_on ) );
	}
