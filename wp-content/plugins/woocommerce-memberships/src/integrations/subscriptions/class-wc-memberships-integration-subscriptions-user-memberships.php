			...
			if ( null !== $subscription_id ) {
				...
				if ( empty( $subscription_id ) ) {
					...
					if ( ! empty( $installment_plan ) ) {
						throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$post_type}_invalid_subscription_data" ), esc_html__( 'Cannot unlink a subscription from a membership and keep an installment plan flag at the same time.', 'woocommerce-memberships' ), 400 );
					}
				...
				// link the membership to a subscription
					...
					if ( ! $subscription ) {
						...
						throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$post_type}_invalid_subscription_data" ), sprintf( esc_html__( 'Subscription%s invalid or not found.', 'woocommerce-memberships' ), is_numeric( $request[ $subscription_param ] ) ? esc_html( ' ' . $request[ $subscription_param ] ) : '' ), 404 );
					}
					...
					if ( null !== $installment_plan ) {
						...
						if ( true === $installment_plan ) {
							...
							throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$post_type}_invalid_subscription_data" ), esc_html__( 'Invalid installment plan flag value.', 'woocommerce-memberships' ), 400 );
						}
					}
				}
				...
				throw new \WC_REST_Exception( esc_html( "woocommerce_rest_{$post_type}_invalid_subscription_data" ), esc_html__( 'Cannot set an installment plan flag for a membership that is not linked to a subscription.', 'woocommerce-memberships' ), 400 );
			}
		}
