...
class WC_Memberships_Integration_Subscriptions_Ajax {
	...
	public function search_subscriptions_by_id_or_customers() {
		...
		$keyword = isset( $_GET['term'] ) ? urldecode( strip_tags( wp_unslash( $_GET['term'] ) ) ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function calculateNewMembershipValuesForSubscriptionId() : void
	{
		...
		$newSubscriptionId = sanitize_text_field( wp_unslash( $_POST['newSubscriptionId'] ?? null ) );
		$currentStatus = sanitize_text_field( wp_unslash( $_POST['currentStatus'] ?? '' ) );
		$currentEndDate = sanitize_text_field( wp_unslash( $_POST['currentEndDate'] ?? '' ) );
