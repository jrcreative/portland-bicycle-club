<?php
require_once __DIR__.'/../app/bootstrap.php';
use Timber\Timber;

// Timber 2.0: Use Timber::context() instead of get_context()
$context = Timber::context();
$template = 'base.html.twig';

if(is_singular())
{
    // Timber 2.0: Use Timber::get_post() directly
    $context['post'] = Timber::get_post();

    if(is_front_page())
    {
        $template = 'pages/page.html.twig';
        $rows = [];
        while(have_rows('content_rows')) {
            the_row();
            if(get_row_layout() == "news") {
                // Timber 2.0: get_posts() with query array directly
                $context['news'] = Timber::get_posts([
                    'posts_per_page' => 8,
                    'orderby' => array(
                        'date' => 'DESC'
                    )
                ]);
            }
            elseif(get_row_layout() == "rides")
            {
                $today = new DateTime(null, new DateTimeZone(pwtc_get_timezone_string()));
                $rides_query = new WP_Query([
                    'posts_per_page' => 6,
                    'post_type' => 'scheduled_rides',
                    'meta_query' => [
                        [
                            'key' => 'date',
                            'value' =>  $today->format('Y-m-d 00:00:00'),
                            'compare' => '>=',
                            'type'  => 'DATETIME'
                        ],
                    ],
                    'orderby' => ['date' => 'ASC'],
                ]);
                $rides_data = [];
                while($rides_query->have_posts()){
                    $rides_query->the_post();
                    $datetime = DateTime::createFromFormat('Y-m-d H:i:s', get_field('date'));
                    $date = $datetime ? $datetime->format('Y-m-d') : '';
                    if(!empty($date) && !isset($rides_data[$date])){
                        $rides_data[$date] = [];
                    }
                    if(!empty($date)) {
                        $rides_data[$date][] = [
                            'title' => get_the_title(),
                            'link' => get_the_permalink(),
                            'date' => get_field('date'),
                            'time' => get_field('date'),
                            'is_canceled' => get_field('is_canceled'),
                        ];
                    }
                }
                // Timber 2.0: Pass WP_Query to get_posts
                $context['rides'] = Timber::get_posts($rides_query);
                $context['news'] = $context['rides'];
                wp_reset_query();
            }
        }
        $context['rows'] = $rows;
    }
    else if(is_single())
    {
        if(is_page())
        {
            $template = 'pages/page.html.twig';
        }
        elseif(get_post_type() == "scheduled_rides")
        {
            // Include the single scheduled rides handler which fetches all ACF fields
            // and then renders the template with proper data
            include __DIR__.'/single-scheduled_rides.php';
            return; // Stop execution - single-scheduled_rides.php handles rendering
        }
        else
        {
            $template = 'two-column.html.twig';
            $context['comments'] = comments_open();
        }
    }
}
elseif(get_post_type() == 'newsletter')
{
    $template = 'pages/archive.html.twig';
    $context['title'] = 'Newsletter Articles';
    if($context['title'] == "Archives") { $context['title'] = "Newsletters"; }
    // Timber 2.0: Use Timber::get_posts() instead of new PostQuery()
    $context['posts'] = Timber::get_posts();
    $context['comments'] = comments_open();
}
else
{
    $template = 'pages/archive.html.twig';
    $context['title'] = get_the_archive_title();
    if($context['title'] == "Archives") { $context['title'] = "News"; }
    // Timber 2.0: Use Timber::get_posts() instead of new PostQuery()
    $context['posts'] = Timber::get_posts();
}

Timber::render($template, $context);
