	...
	<nav class="woocommerce-MyAccount-navigation wc-memberships-members-area-navigation">
        <ul class="vertical tabs" data-tabs id="myaccount-tabs">
			<?php foreach ( $members_area_sections as $section_id => $section_data ) : ?>
				<li class="tabs-title <?php echo esc_attr( wc_get_account_menu_item_classes( $section_id ) . ' ' . $section_data['class'] ); ?>">
					<a
                        href="<?php echo esc_url( $section_data['url'] ); ?>"
                        <?php if(strpos(wc_get_account_menu_item_classes( $section_id ) . ' ' . $section_data['class'], 'is-active')):?> aria-selected="true"<?php endif ?>
